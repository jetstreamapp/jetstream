...
/** Counts seat usage straight from the database. */
export async function getTeamSeatSummary(
  db: SeatDbClient,
  { teamId, now = new Date() }: { teamId: string; now?: Date },
...
export async function assertSeatAvailable(db: SeatDbClient, { teamId, kind, role }: SeatCheckParams): Promise<TeamSeatSummary> {
  const { billingStatus, seats } = await getTeamSeatSummary(db, { teamId });
