...
export async function addMemberFromInvitation(
...
): Promise<{ role: string; status: string; features: Prisma.JsonValue }> {
  ...
  if (isBillableRole(role)) {
    await assertSeatAvailable(tx, { teamId, kind: 'ACCEPT_INVITATION', role });
  }
  await tx.teamMemberInvitation.delete({ where: { id: invitation.id } });
  // Role and features come from the locked re-read, so callers that report what was accepted (the
  // audit log) describe the membership that was actually created rather than their stale snapshot.
  return tx.teamMember.create({
    select: { role: true, status: true, features: true },
    data: {
      teamId,
      userId,
      role,
