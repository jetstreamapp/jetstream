...
function mockTeam({ licenseCountLimit = 5, pendingSeatQuantity = null as number | null, billingStatus = 'ACTIVE' } = {}) {
  mocks.teamFindUniqueOrThrow.mockResolvedValue({
    billingStatus,
    billingAccount: { licenseCountLimit, pendingSeatQuantity, pendingSeatEffectiveAt: null },
  });
}
...
describe('getTeamSeatSummary', () => {
  ...
  it('counts active billable members and unexpired billable invitations', async () => {
    mockTeam({ licenseCountLimit: 5 });
    mocks.teamMemberCount.mockResolvedValue(3);
    mocks.invitationCount.mockResolvedValue(1);

    const now = new Date('2026-09-10T00:00:00Z');
    const { seats } = await getTeamSeatSummary(prisma, { teamId: 'team-1', now });
    ...
    expect(mocks.teamMemberCount).toHaveBeenCalledWith({
      where: { teamId: 'team-1', status: 'ACTIVE', role: { in: ['ADMIN', 'MEMBER'] } },
    });
  ...
  it('is unlimited when the team has no billing account', async () => {
    ...
    const { seats } = await getTeamSeatSummary(prisma, { teamId: 'team-1' });
...
describe('assertSeatAvailable', () => {
  ...
  it('carries the role being granted and exposes the API-facing status and additionalData without an import', async () => {
    mockTeam({ licenseCountLimit: 1 });
    mocks.teamMemberCount.mockResolvedValue(1);
    mocks.invitationCount.mockResolvedValue(0);

    const error: SeatLimitError = await assertSeatAvailable(prisma, { teamId: 'team-1', kind: 'ADD', role: 'ADMIN' }).catch((ex) => ex);
    expect(error.role).toBe('ADMIN');
    expect(error.status).toBe(400);
    expect(error.additionalData).toEqual({ code: 'NO_SEATS', kind: 'ADD', role: 'ADMIN', seats: error.seats });
  });
