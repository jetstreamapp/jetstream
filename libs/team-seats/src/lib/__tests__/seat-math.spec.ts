...
describe('validateRequestedSeatCount', () => {
  const base = { seats: summary(), includedSeats: 0 };
  ...
  it('accepts a count equal to current usage, the current count, and any increase', () => {
    expect(validateRequestedSeatCount({ ...base, requested: 4 })).toEqual({ ok: true });
    expect(validateRequestedSeatCount({ ...base, requested: 5 })).toEqual({ ok: true });
