...
/**
 * Records a membership change that was refused for lack of a seat, or because the team is past due, so
 * admins can see who was turned away in the team audit log. Every blocked path writes the same entry through here so the metadata
...
export function auditSeatLimitRejection(error: SeatLimitError, context: SeatLimitRejectionContext): void {
  ...
  createTeamAuditLog({
    userId,
    teamId: error.teamId,
    action: error.code === 'PAST_DUE' ? AuditLogAction.TEAM_MEMBER_ADD_BLOCKED_PAST_DUE : AuditLogAction.TEAM_MEMBER_ADD_BLOCKED_NO_SEATS,
