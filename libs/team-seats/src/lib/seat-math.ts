import { BILLABLE_ROLES, MAX_TEAM_SEATS, TEAM_MEMBER_STATUS_ACTIVE, TeamSeatCheckKind, TeamSeatSummary } from '@jetstream/types';
...
export type SeatChangeRejection = 'MIN_SEATS' | 'MAX_SEATS' | 'BELOW_USAGE' | 'BELOW_INCLUDED';

export type SeatChangeValidation = { ok: true } | { ok: false; code: SeatChangeRejection; detail?: Record<string, number> };

/**
 * Whether a team may move to `requested` seats. Billing standing (manual billing, past due) is the
 * caller's gate and whether the count is actually a change is classification, so neither lives here.
 */
