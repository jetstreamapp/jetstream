...
export function TeamLoginConfiguration({ loginConfiguration, hasSsoConfigured, ssoIsActive, onUpdate }: TeamLoginConfigurationProps) {
  ...
  async function onSubmit(ev: React.SubmitEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // The baseline is the submitted payload, not the live form: the spinner does not block the
      // keyboard, and a setting toggled mid-save was never sent so it has to stay dirty.
      setFormData((prev) => ({
        data: prev.data,
        originalData: {
          ...payload,
          allowedMfaMethods: new Set(payload.allowedMfaMethods),
          allowedProviders: new Set(payload.allowedProviders),
        },
      }));
    } catch (ex) {
      fireToast({
