...
import { getApiErrorCode, previewTeamSeats, updateTeamSeats } from '@jetstream/shared/data';
...
import { formatSeatDate, hasPendingSeatDecrease } from './team-seats.utils';
...
export function TeamSeatsManageModal({ teamId, seats, onClose }: TeamSeatsManageModalProps) {
  ...
  // The same floor the server enforces: usage, plus whatever a legacy flat-first-tier price includes
  const minSeats = getMinimumSeats(seats);
  ...
  const changeType = classifySeatChange({ requested: requestedSeats, purchased: purchasedSeats, pending: seats.pending });
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      // Only a rejected preview is fixed by previewing again; a declined card or a manual-billing
      // rejection carries its own remedy, so appending the hint there would send the admin in circles
      const isStalePreview = STALE_PREVIEW_CODES.has(getApiErrorCode(ex) ?? '');
      setErrorMessage(isStalePreview ? `${getErrorMessage(ex)} Go back and preview again.` : getErrorMessage(ex));
    } finally {
      setLoading(false);
    }
  }
