...
export const QuerySubqueryConfigPanel: FunctionComponent<QuerySubqueryConfigPanelProps> = ({
  ...
  useEffect(() => {
    ...
    // Retry when the prior fetch left an error entry — otherwise reopening the panel would
    // short-circuit on the non-empty error entry, contradicting the "close and reopen to try
    // again" message shown to the user.
    const existingEntry = queryFieldsMap[childBaseKey];
    if (!isEmpty(existingEntry) && !existingEntry?.hasError) {
      return;
    }
  ...
  const baseFieldsEntry = queryFieldsMap[childBaseKey];
  const hasBaseFieldsError = !!baseFieldsEntry?.hasError;
  const hasChildMetadata = !isEmpty(baseFieldsEntry) && !baseFieldsEntry?.loading && !hasBaseFieldsError;
    ...
    >
      ...
      >
        ...
        >
          ...
          {hasBaseFieldsError && !isFetchingBaseFields && (
            <p className="slds-text-body_small slds-text-color_error slds-m-bottom_medium">
              There was an error loading fields for this object. Close and reopen the panel to try again.
            </p>
          )}
          {!hasChildMetadata && !hasBaseFieldsError && !isFetchingBaseFields && (
