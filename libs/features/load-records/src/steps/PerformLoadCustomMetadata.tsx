...
import { groupByFlat, isProductionOrg } from '@jetstream/shared/utils';
import {
  DeployMessage,
  DownloadModalData,
  DownloadType,
  ...
  async function handleStartLoad() {
    if (
      ...
      try {
        ...
        tracker.error('Problem preparing custom metadata', ex);
      }
    }
  }
  ...
  return (
    <div>
      ...
      <LoadRecordsDuplicateWarning
        ...
        isCustomMetadata
      />
      <h2 className="slds-text-heading_medium">Options</h2>
      <div className="slds-p-around_small">
        ...
        >
          <select className="slds-select" id="date-format" required value={dateFormat} disabled={loading} onChange={handleDateFormatChange}>
      ...
      <h2 className="slds-text-heading_medium">Summary</h2>
      <div className="slds-p-around_small">
        <div>
        ...
        <div className="slds-m-top_small">
          ...
          <button className="slds-button slds-button_brand" {...ariaDisabledButtonProps(loading, () => handleStartLoad())}>
            Upsert <strong className="slds-m-horizontal_xx-small">{numRecordsImpactedLabel}</strong> Records
          </button>
        </div>
      </div>
      <h2 className="slds-text-heading_medium">Results</h2>
      <div className="slds-p-around_small">
        {/* Progress and failures render inline with no focus change — live regions announce them. The
            upload status is mirrored into a persistent region: a live-region role on the heading itself
            removed its heading semantics, and a region that mounts already containing text is skipped */}
        <AssistiveStatus message={hasLoaded && !results ? 'Uploading custom metadata' : ''} />
        {hasLoaded && !results && (
