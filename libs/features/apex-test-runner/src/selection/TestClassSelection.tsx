...
import TestClassSelectionRow, { ClassSelection, getTabbableIdInRow, methodRovingId } from './TestClassSelectionRow';
...
export const TestClassSelection: FunctionComponent<TestClassSelectionProps> = ({
  ...
  // Stable so the memoized rows are not re-rendered by it
  const handleToggleExpand = useCallback((classId: string) => {
    setExpandedClasses((prior) => {
      const updated = new Set(prior);
        ...
        <ul aria-label="Test classes" {...classList.containerProps}>
          {filteredTestClasses.map((item) => (
            <TestClassSelectionRow
              ...
              tabbableIdInRow={getTabbableIdInRow(item.classId, classList.tabbableId)}
              onItemFocus={classList.onItemFocus}
              ...
              <ul aria-label="Classes that could not be analyzed" {...unknownClassList.containerProps}>
                {filteredUnknownClasses.map((item) => (
                  <TestClassSelectionRow
                    ...
                    selection={getSelection(item.classId)}
                    expanded={false}
                    tabbableIdInRow={getTabbableIdInRow(item.classId, unknownClassList.tabbableId)}
                    onItemFocus={unknownClassList.onItemFocus}
                    onToggleExpand={handleToggleExpand}
                    onToggleClass={onToggleClass}
                    onToggleMethod={onToggleMethod}
                  />
