...
function toWorkbookError(message: string): LoadMultiObjectDataError {
  return {
    property: null,
    // Matches the worksheet `parseWorkbook` uses for its own file-level problems, so both group under one heading
    worksheet: 'Workbook',
    location: null,
    locationType: 'SHEET',
    message,
  ...
  const processFile = useCallback(
    async (source: ArrayBuffer) => {
      ...
      try {
        ...
        // Nothing parsed, so there are no worksheets to preview - the review step renders the workbook error on its
        // own so the reason outlives the toast
