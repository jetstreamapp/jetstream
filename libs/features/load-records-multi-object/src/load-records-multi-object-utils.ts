...
import {
  formatNumber,
  LOAD_MULTI_OBJECT_TEMPLATE_LAYOUT,
  openLoadMultiObjectTemplateWorkbook,
  type LoadMultiObjectTemplateSheet,
...
const { cells: TEMPLATE_CELLS, headerRow: TEMPLATE_HEADER_ROW } = LOAD_MULTI_OBJECT_TEMPLATE_LAYOUT;

/** Where the error messages point - derived from the template layout so the reader and the messages can never disagree */
const WORKSHEET_LOCATIONS = {
  sobject: TEMPLATE_CELLS.sobject,
  operation: TEMPLATE_CELLS.operation,
  externalId: TEMPLATE_CELLS.externalId,
  referenceId: TEMPLATE_CELLS.referenceIdHeader,
  dataStartCell: TEMPLATE_CELLS.referenceIdHeader,
  headerRow: TEMPLATE_HEADER_ROW,
};
...
function getExcelRow(recordIdx: number) {
  return WORKSHEET_LOCATIONS.headerRow + 1 + recordIdx;
}
...
function buildDataset(sheetName: string, sheet: LoadMultiObjectTemplateSheet): LoadMultiObjectData {
  ...
  if (populatedHeaders.length !== new Set(populatedHeaders).size) {
    errors.push({
      property: 'data',
      worksheet: sheetName,
      location: `${WORKSHEET_LOCATIONS.headerRow}`,
      locationType: 'ROW',
    ...
    errors: [
      {
        ...
        locationType: 'CELL',
        message: `Jetstream could not read the data on this worksheet. Check that headers are on row ${WORKSHEET_LOCATIONS.headerRow}, data starts on row ${WORKSHEET_LOCATIONS.headerRow + 1}, and the sheet matches the template layout.`,
      },
    ],
  };
  return dataset as LoadMultiObjectData;
...
export async function parseWorkbook(source: ArrayBuffer | Uint8Array | Blob, org: SalesforceOrgUi): Promise<ParseWorkbookResult> {
  ...
  const unreadableSheets = new Set<string>();
  ...
  try {
    ...
    for (const sheetName of sheetNames) {
      try {
        ...
        unreadableSheets.add(sheetName);
      }
    }
  } finally {
  ...
  // An unreadable sheet has no object or operation to check, and already carries the one error that matters
  await validateObjectData(
    org,
    datasets.filter(({ worksheet }) => !unreadableSheets.has(worksheet)),
  );
