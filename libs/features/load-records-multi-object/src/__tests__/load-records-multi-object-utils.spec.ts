...
describe('parseWorkbook', () => {
  ...
  it('reads an error value in cell B1 as a missing object name, not as an object called "#REF!"', async () => {
    const workbook = await buildWorkbook({
      Sheet1: [['Object Api Name', { error: '#REF!' }], ['Operation', 'Insert'], [], [], ['Reference Id', 'Name'], ['rec1', 'Test']],
    });
    const datasets = await parseDatasets(workbook);
    ...
    expect(error.location).toBe('B1');
    expect(error.message).toContain('Cell B1 must contain the Object API Name');
    expect(describeSObjectMock).not.toHaveBeenCalled();
  });
