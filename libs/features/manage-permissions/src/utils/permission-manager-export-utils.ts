...
async function writeWorksheet(
  ...
  sheetName: string,
  { headerRows, dataRows, merges, dateColumnIndexes }: PermissionWorksheet,
  ...
  // `rowCount` counts the header rows too, since they are written as ordinary rows - knowing the size up front is
  // what lets the writer switch to zip64 for a huge sheet instead of failing once it passes 4 GiB
  const sheet = workbook.addSheet(sheetName, {
    columns: getExcelColumnWidths(headerRows[headerRows.length - 1], dataRows),
    rowCount: headerRows.length + dataRows.length,
  });
