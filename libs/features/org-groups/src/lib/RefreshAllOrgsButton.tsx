...
export const RefreshAllOrgsButton = ({ className, orgs }: RefreshAllOrgsButtonProps) => {
  ...
  return (
    <>
      ...
      {/* Announced once when the refresh starts (a per-org count would queue an announcement for every
          completion); the completion toast announces the outcome */}
      <AssistiveStatus message={isRefreshing ? `Refreshing ${orgs.length} orgs` : ''} />
    </>
  );
};
