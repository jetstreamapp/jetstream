...
function getAppCookie(): ApplicationCookie {
  let appState = (window as any).electron?.isElectron
    ? (window as any).electron.appCookie
    : parseCookie<ApplicationCookie>(HTTP.COOKIE.JETSTREAM);
  appState = appState
    ? { ...appState }
    : {
  ...
  if ((window as any).electron?.isElectron) {
    ...
    appState.environment = (window as any).electron?.isElectronDev ? 'development' : 'production';
  }
  return appState;
}
...
export const electronPreferences = atom<ElectronPreferences>({
  ...
  default: (window as any).electron?.initialPreferences,
});
