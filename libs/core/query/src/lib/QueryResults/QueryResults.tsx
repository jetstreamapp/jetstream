...
import { getFlattenSubqueryFlattenedFieldMap } from '@jetstream/shared/query-utils';
import {
  formatNumber,
  hasModifierKey,
  isMKey,
...
export const QueryResults: FunctionComponent<QueryResultsProps> = React.memo(({ selectedOrg }) => {
  ...
  const [restore] = useQueryRestore(selectedOrg, soql, isTooling, { silent: true });
  ...
  function handleRestoreFromHistory(soql: string, tooling: boolean) {
