...
export type TeamUserAction = 'deactivate' | 'enable';
...
const TeamMemberRoleSchema = z.enum(['ADMIN', 'BILLING', 'MEMBER']);
const TeamMemberStatusSchema = z.enum(['ACTIVE', 'INACTIVE']);
...
export const TeamLoginConfigSchema = z.object({
  ...
  domains: z.string().array().optional().default([]),
  requireMfa: z.boolean().optional().default(false),
});
export const TeamLoginConfigRequestSchema = TeamLoginConfigSchema.omit({ domains: true });
