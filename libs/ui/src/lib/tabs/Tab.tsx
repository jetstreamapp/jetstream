...
export const Tab = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* The link is inline-flex, where text-overflow never applies to its own text — a text label
            needs a block of its own to end in an ellipsis rather than a hard clip */}
        {truncateLabels && typeof tab.title === 'string' ? <span className="slds-truncate">{highlightedHeading}</span> : highlightedHeading}
      </a>
    </li>
  );
};
