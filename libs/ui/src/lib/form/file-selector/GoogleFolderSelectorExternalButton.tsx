...
export interface GoogleFolderSelectorExternalButtonProps {
  // Accepted for prop compatibility with GoogleFolderSelectorProps spread, but unused in desktop flow
...
export const GoogleFolderSelectorExternalButton: FunctionComponent<GoogleFolderSelectorExternalButtonProps> = ({
  ...
  const [selectedFolder, setSelectedFolder] = useState<google.picker.DocumentObject | undefined>();
  ...
  const handlePickerResult = useCallback(
    (result: GooglePickerResultSuccess) => {
      ...
      try {
        ...
        const syntheticDoc = { id: folderId, name: resultFolderName } as google.picker.DocumentObject;

        setSelectedFolder(syntheticDoc);
        setManagedName(resultFolderName);
        onSelected(syntheticDoc);
      } catch (ex) {
