...
const defaultGroupFilterFn = (filter: string) => multiWordObjectFilter<ListItemGroup>(['label'], filter);
...
export const ComboboxWithGroupedItems = forwardRef<ComboboxWithGroupedItemsRef, ComboboxWithGroupedItemsProps>(
  (
    ...
    const itemsById = useMemo(() => getFlattenedListItemsById(groups.flatMap((group) => group.items)), [groups]);
