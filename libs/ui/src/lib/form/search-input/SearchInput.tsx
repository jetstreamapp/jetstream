...
export const SearchInput: FunctionComponent<SearchInputProps> = ({
  ...
  // Whether the arrow press in flight started on this input. The Up/Down action fires on keyup, but a
  // keyup only means something when this input also saw the keydown: when another control (Select All
  // above a field list) moves focus here on ITS keydown, the same press's keyup lands here and must
  // not bounce focus straight back out into the list.
  const arrowPressRef = useRef(false);
  ...
  function handleKeyUp(event: KeyboardEvent<HTMLInputElement>) {
    if (onArrowKeyUpDown) {
      ...
      if (direction) {
        event.preventDefault();
        event.stopPropagation();
        if (!arrowPressRef.current) {
          return;
        }
        arrowPressRef.current = false;
