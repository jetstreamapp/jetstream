 ...
 * (closing a layer, leaving the editor): its own overlays, and extra cursors that collapse on Escape.
 *
 ...
 * that has an overlay open (autocomplete, parameter hints, find, hover, rename, snippet) or more than
 * one cursor. Monaco dismisses that state itself and stops the event, so a capture-phase Escape handler
