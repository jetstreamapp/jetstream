...
describe('List', () => {
  test('renders all item headings', () => {
    render(<List items={items} isActive={() => false} getContent={getContent} onSelected={() => {}} />);
  ...
  test('calls onSelected with the key when an item is clicked', () => {
    ...
    render(<List items={items} isActive={() => false} getContent={getContent} onSelected={onSelected} />);
  ...
  test('renders nothing when items is empty', () => {
    render(<List items={[]} isActive={() => false} getContent={getContent} onSelected={() => {}} />);
  ...
  test('keyboard Home moves focus to first item', () => {
    render(<List items={items} isActive={(item) => item.id === 'item-3'} getContent={getContent} onSelected={() => {}} />);
    const listbox = screen.getByRole('listbox');
    const options = screen.getAllByRole('option');
...
describe('ReadonlyList', () => {
  test('renders all item headings', () => {
    render(<ReadonlyList items={items} getContent={getContent} />);
    expect(screen.getByText('Alpha')).toBeTruthy();
    expect(screen.getByText('Beta')).toBeTruthy();
  });
  ...
  test('renders nothing when items is empty', () => {
    const { container } = render(<ReadonlyList items={[]} getContent={getContent} />);
