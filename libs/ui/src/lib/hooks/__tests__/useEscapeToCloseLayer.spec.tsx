...
/**
 * A popover-like layer that knows its own element, next to a modal dialog. `layerInsideModal` renders the
 * layer within the dialog (a filter popover in a modal grid); otherwise it sits behind it (a record
 * popover whose "View Record" opened the modal and stayed open underneath).
 */
function LayerAndModalHarness({ onEscape, layerInsideModal }: { onEscape: () => void; layerInsideModal: boolean }) {
  ...
  useEscapeToCloseLayer(
    ...
    { getLayerElement: () => layerElement },
  );
  const layer = (
    <div ref={setLayerElement} data-testid="layer">
      Popover content
    </div>
  );
  return (
    <div>
      {!layerInsideModal && layer}
      <div role="dialog" aria-modal="true" aria-label="Record">
        {layerInsideModal && layer}
...
describe('useEscapeToCloseLayer', () => {
  ...
  it('ignores the Escape that cancels an IME conversion, including Safari where isComposing is already false', () => {
    const onEscape = vi.fn();
    render(<SingleLayerHarness onEscape={onEscape} />);
    const target = screen.getByRole('button', { name: 'Key target' });

    fireEvent.keyDown(target, { key: 'Escape', isComposing: true });
    fireEvent.keyDown(target, { key: 'Escape', keyCode: 229 });
  ...
  it('leaves Escape to a modal that opened over the layer, so one press closes the modal', () => {
    const onEscape = vi.fn();
    const documentKeyDown = listenOnDocument('keydown');
    const documentKeyUp = listenOnDocument('keyup');
    render(<LayerAndModalHarness onEscape={onEscape} layerInsideModal={false} />);

    pressEscape(screen.getByRole('button', { name: 'Modal control' }));

    expect(onEscape).not.toHaveBeenCalled();
    expect(screen.getByTestId('layer-state').textContent).toBe('open');
    // Neither phase is consumed: floating-ui closes the modal on keydown, Modal itself on keyup
    expect(documentKeyDown).toHaveBeenCalledTimes(1);
    expect(documentKeyUp).toHaveBeenCalledTimes(1);
  });

  it('still closes a layer that lives inside the modal, without the modal seeing the press', () => {
    const onEscape = vi.fn();
    const documentKeyDown = listenOnDocument('keydown');
    render(<LayerAndModalHarness onEscape={onEscape} layerInsideModal />);
  ...
  it('leaves Escape to a code editor whose autocomplete is open, instead of closing the layer', () => {
    const onEscape = vi.fn();
    render(<SingleLayerHarness onEscape={onEscape} />);
    // A Monaco editor inside the layer (Quick Query, manual SOQL) with its suggest widget open
    const editor = document.createElement('div');
    editor.className = 'monaco-editor';
    editor.innerHTML = '<textarea class="inputarea"></textarea><div class="suggest-widget visible"></div>';
    ...
    // Once Monaco has dismissed its widget, the same press is the layer's again
    editor.querySelector('.suggest-widget')?.remove();
    pressEscape(textarea);
    expect(onEscape).toHaveBeenCalledTimes(1);
    expect(screen.getByTestId('layer-state').textContent).toBe('closed');
    editor.remove();
  ...
  it('leaves Escape to a code editor with selected text, which the press deselects (Quick Query opens fully selected)', () => {
    ...
    const editor = document.createElement('div');
    editor.className = 'monaco-editor';
    editor.innerHTML = '<textarea class="inputarea"></textarea><div class="view-overlays"><div class="cslr selected-text"></div></div>';
    document.body.appendChild(editor);
    const textarea = editor.querySelector('textarea') as HTMLTextAreaElement;
    ...
    editor.querySelector('.selected-text')?.remove();
    pressEscape(textarea);
    expect(onEscape).toHaveBeenCalledTimes(1);
    editor.remove();
  });
