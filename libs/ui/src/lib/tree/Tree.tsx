...
export const Tree = forwardRef<any, TreeProps>(
  (
    ...
    // Per instance: two trees on one page (deploy view/compare) must not share a heading id
    ...
    return (
      <div className={classNames('slds-tree_container', className)}>
        {header && (
          <h4 className="slds-tree__group-header" id={headingId}>
            {header}
          </h4>
        )}
        {/* Composite-widget pattern: the ul delegates keyboard handling for its treeitem descendants */}
        {/* eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions */}
        <ul ref={ulRef} aria-labelledby={header ? headingId : undefined} className="slds-tree" role="tree" onKeyDown={handleKeyDown}>
          {items.map((item) => (
            <TreeItem
              key={`0-${item.id}`}
              item={item}
