...
describe('getIncludedSeatsFromPriceTiers', () => {
  ...
  it('reads a flat first tier whose unit amount Stripe reports as 0', () => {
    expect(getIncludedSeatsFromPriceTiers([{ flatAmount: 1100, unitAmount: 0, upTo: 5 }])).toBe(5);
  });

  it('includes nothing when the first tier also charges per seat', () => {
    expect(getIncludedSeatsFromPriceTiers([{ flatAmount: 0, unitAmount: 25, upTo: 10 }])).toBe(0);
    expect(getIncludedSeatsFromPriceTiers([{ flatAmount: 100, unitAmount: 25, upTo: 10 }])).toBe(0);
  });
