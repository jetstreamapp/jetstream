...
export function getIncludedSeatsFromPriceTiers(tiers: readonly JetstreamPriceTier[] | null | undefined): number {
  ...
  // and reading that as "no flat amount" would shrink the team's cap to the raw item quantity. A tier
  // that also charges per seat is a per-seat tier with a flat fee on top, not included seats; Stripe
  // reports a flat-only tier's unit amount as 0 rather than null, so only a positive amount counts.
  if (!firstTier || firstTier.flatAmount === null || !!firstTier.unitAmount || firstTier.upTo === null) {
    return 0;
  }
 ...
 * whatever the plan includes for free, and never below one. Shared so the seat modal's stepper, the
 * checkout seat picker and the server's validation cannot disagree about the floor.
 *
 * Checkout passes `includedSeats: null`: a team buying its first subscription has no included seats
