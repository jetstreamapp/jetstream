...
import {
  ...
  ValidationResults,
} from './mass-update-records.types';
...
export interface MassUpdateRecordsObjectRowProps {
   ...
   */
  recordLimit?: RecordLimitAndOffset & { onChange: (limitAndOffset: RecordLimitAndOffset) => void };
  disabled?: boolean;
...
export const MassUpdateRecordsObjectRow: FunctionComponent<MassUpdateRecordsObjectRowProps> = ({
  ...
  async function handleDownloadValidationRecords() {
    try {
      ...
      const row = { sobject, configuration: fieldConfigurations, limit: recordLimit?.limit, offset: recordLimit?.offset } as MetadataRow;
      // Shares the deploy path's fetch so the previewed records are exactly the records that will be updated
      const { records } = await queryRecordsForRow(row, fieldsToQuery, org);
