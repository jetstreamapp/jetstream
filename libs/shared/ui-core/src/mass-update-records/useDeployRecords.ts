      ...
      for (const batch of batches) {
        try {
          ...
          if (!errorMessage.includes('aborted') && !errorMessage.includes('limit exceeded')) {
            tracker.error('There was an error loading batch for mass record update', { message: ex.message, stack: ex.stack });
          }
    ...
    async (rows: MetadataRow[], options: { batchSize: number; serialMode: boolean }) => {
      ...
      for (const row of rows) {
        try {
          ...
          tracker.error('There was an error loading data for mass record update', { message: ex.message, stack: ex.stack });
      ...
      for (const row of rows) {
        if (!row.deployResults.done && row.deployResults.jobInfo?.id) {
          try {
            ...
            tracker.error('There was an error polling bulk api job', { message: ex.message, stack: ex.stack });
            const deployResults: DeployResults = {
              ...row.deployResults,
              done: true,
              status: 'Error',
  ...
  const pollResultsUntilDone = useCallback(
    async (getRows: () => Pick<MetadataRow, 'deployResults' | 'sobject'>[]) => {
      try {
        ...
        tracker.error('There was an error polling for mass record update results', { message: ex.message, stack: ex.stack });
