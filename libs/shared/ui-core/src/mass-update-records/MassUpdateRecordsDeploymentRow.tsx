...
} & Pick<MetadataRow, 'sobject' | 'deployResults' | 'configuration' | 'limit' | 'offset'>;
...
export const MassUpdateRecordsDeploymentRow = ({
  ...
  const { limit: effectiveLimit, offset: effectiveOffset } = getEffectiveRecordLimit({ limit, offset });
  ...
  return (
    <Fragment>
      ...
      >
        ...
        {!processingStartTime && !!effectiveOffset && (
          <div className="slds-m-left_medium">
            The first <span className="text-bold">{formatNumber(effectiveOffset)}</span> matching{' '}
            {pluralizeFromNumber('record', effectiveOffset)} will be skipped.
          </div>
        )}
