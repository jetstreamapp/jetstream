...
/** Minimal stand-in for the editor object Monaco hands to `onMount`. A bare cursor is an EMPTY selection. */
function mountEditor(domNode: HTMLElement, { selectionCount = 1, hasSelectedText = false } = {}) {
  lastEditorProps().onMount?.(
    {
      getDomNode: () => domNode,
      getSelections: () => Array.from({ length: selectionCount }, () => ({ isEmpty: () => !hasSelectedText })),
    } as never,
    {} as never,
  );
}
...
describe('MonacoEditor', () => {
  ...
  it('leaves Escape to Monaco while there are extra cursors to collapse', () => {
    ...
    render(<MonacoEditor />);
    mountEditor(document.createElement('div'), { selectionCount: 2 });
    pressEscape();

    expect(document.activeElement?.id).not.toBe('after');
  });

  // Monaco binds Escape to cancelSelection: leaving on that same press sent the next keystrokes to
  // whatever control follows the editor
  it('leaves Escape to Monaco while text is selected, so the press only cancels the selection', () => {
    document.body.innerHTML = `<button id="after">Execute</button>`;

    render(<MonacoEditor />);
    mountEditor(document.createElement('div'), { hasSelectedText: true });
    pressEscape();
