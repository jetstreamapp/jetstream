...
import { focusNextTabbableAfter, focusPreviousTabbableBefore, isImeComposing } from '@jetstream/shared/ui-utils';
...
function useExitOnEscape(onMount: OnMount | undefined) {
  ...
  const handleKeyDownCapture = useCallback((event: KeyboardEvent<HTMLDivElement>) => {
    ...
    if (event.key !== 'Escape' || isImeComposing(event.nativeEvent) || shouldMonacoKeepEscape(editorRef.current, event.target)) {
      return;
    }
