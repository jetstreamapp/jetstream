...
/** Escape still belongs to Monaco while one of its overlays is open or extra cursors remain to collapse. */
function shouldMonacoKeepEscape(editorInstance: editor.IStandaloneCodeEditor | null): boolean {
  const editorNode = editorInstance?.getDomNode();
  if (!editorNode) {
    return true;
  }
  return monacoEditorOwnsEscape(editorNode) || (editorInstance?.getSelections()?.length ?? 1) > 1;
}
...
function useExitOnEscape(onMount: OnMount | undefined) {
  ...
  const handleKeyDownCapture = useCallback((event: KeyboardEvent<HTMLDivElement>) => {
    if (event.key !== 'Escape' || shouldMonacoKeepEscape(editorRef.current)) {
      return;
    }
