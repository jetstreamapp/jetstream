...
describe('parseSalesforceLoginUrl', () => {
  ...
  describe('legacy instance-scoped sandbox hosts', () => {
    // Pre-enhanced-domains sandboxes kept the instance in the host - the `--` still marks a sandbox
    it('detects the sandbox from the -- even though the environment segment is an instance', () => {
      expect(parseSalesforceLoginUrl('acme--uat.cs123.my.salesforce.com')).toEqual({
        success: true,
        myDomain: 'acme--uat.cs123',
        isSandbox: true,
        loginUrl: 'https://acme--uat.cs123.my.salesforce.com',
      });
    });
  });
