...
describe('OrgActivitySync', () => {
  ...
  /**
   * Orgs connected before `lastActivityAt` existed have no date to count from, so nothing about them
   * is day-dependent and the daily refresh skipped them. The server records the column as part of
   * confirming the activity, so the first confirmed use has to put one on the client too.
   */
  it('records the first confirmed activity for an org that has never had any', () => {
    const legacy = buildOrg('legacy');
    const { orgById } = renderWithOrgs([legacy]);

    act(() => notifyOrgActivity(legacy));

    expect(orgById('legacy')?.lastActivityAt).toBeTruthy();
  });
