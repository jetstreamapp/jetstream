...
export interface ParsedLoginUrlSuccess {
   ...
   * Always re-parses to the same `loginUrl`, so it is safe to use as an editable prefill.
   */
  myDomain: string;
  isSandbox: boolean;
...
function extractHostname(input: string): string {
  let value = input.trim().toLocaleLowerCase();
...
export function parseSalesforceLoginUrl(input: string): ParsedLoginUrl {
  ...
  return {
    success: true,
    myDomain,
    isSandbox: myDomain.endsWith(`.${SANDBOX_SEGMENT}`) || (!myDomain.includes('.') && myDomain.includes('--')),
