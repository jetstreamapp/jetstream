...
export function useLoadGoogleApi({
  ...
  const initClient = useCallback(async (scope: string, forceInit?: boolean) => {
    try {
      ...
      if (isMounted.current) {
        ...
        tracker.critical('Error loading Google API', ex, { googleErrorCode: ex.error, details: ex.details });
  ...
  const signOut = useCallback(() => {
    try {
      ...
      tracker.critical('Google Sign Out error (could be user initiated)', ex, { googleErrorCode: ex.error });
    }
  }, [signedIn]);
