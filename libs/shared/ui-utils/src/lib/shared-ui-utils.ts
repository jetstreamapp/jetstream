...
export interface LoadMultiObjectTemplateOptions {
  ...
  fields: string[];
  records: any[];
  /** Map of subquery relationship name to the child fields in the query, from `getFlattenSubqueryFlattenedFieldMap` */
  subqueryFields?: Record<string, string[]>;
  /** Child relationships from the parent object describe - resolves the lookup field that links each child back to its parent */
 ...
 */
function getSubqueryRecords(record: any, relationshipName: string): any[] {
  if (!record) {
    return [];
  }
 ...
 */
function getChildRelationshipForSubquery(childRelationships: ChildRelationship[], relationshipName: string): Maybe<ChildRelationship> {
...
export interface LoadMultiObjectTemplatePlan {
  ...
  linked: { relationshipName: string; childRelationship: ChildRelationship }[];
  /** Subqueries with no matching child relationship on the parent - the linking field is unknown, so they are omitted */
  skipped: string[];
...
/**
 * What `prepareLoadMultiObjectTemplate` will produce for these options, without building any rows.
 *
 ...
 */
export function planLoadMultiObjectTemplate({
  sobject,
  fields,
  records,
  subqueryFields = {},
  childRelationships = [],
}: LoadMultiObjectTemplateOptions): LoadMultiObjectTemplatePlan {
  const linked: LoadMultiObjectTemplatePlan['linked'] = [];
  const skipped: string[] = [];
  const missingReferenceId = hasIdField(fields) ? [] : [sobject];

  Object.entries(subqueryFields).forEach(([relationshipName, childFields]) => {
    const childRelationship = getChildRelationshipForSubquery(childRelationships, relationshipName);
    if (!childRelationship) {
      skipped.push(relationshipName);
      return;
    }
    linked.push({ relationshipName, childRelationship });
    if (!hasIdField(childFields)) {
      missingReferenceId.push(relationshipName);
    }
  });

  // A parent counts itself plus everything it brings with it, since they all load as a single group
  const largestGroupSize = linked.length
    ? records.reduce(
        (largestSize, record) =>
          Math.max(
            largestSize,
            linked.reduce((size, { relationshipName }) => size + getSubqueryRecords(record, relationshipName).length, 1),
          ),
        0,
      )
    : 0;

  return { linked, skipped, missingReferenceId, largestGroupSize };
}
...
export function prepareLoadMultiObjectTemplate(options: LoadMultiObjectTemplateOptions): Record<string, any[][]> {
  ...
  const { linked } = planLoadMultiObjectTemplate(options);
  const templateFields = getLoadMultiObjectTemplateFields(fields, records, new Set(Object.keys(subqueryFields)));

  const output: Record<string, any[][]> = {
    [getExcelSafeSheetName(sobject.replace(EXCEL_FORBIDDEN_SHEET_NAME_CHARS, ''))]: [
      ...getLoadMultiObjectTemplateHeaderRows(sobject),
      ['Reference Id', ...templateFields],
      ...records.map((record) => [record.Id ?? '', ...templateFields.map((field) => getLoadMultiObjectTemplateCellValue(record[field]))]),
    ],
  };

  linked.forEach(({ relationshipName, childRelationship }) => {
    const childRecordsWithParentId = records.flatMap((record) =>
      getSubqueryRecords(record, relationshipName).map((childRecord) => ({ parentId: record.Id ?? '', childRecord })),
    );
    ...
    const childTemplateFields = getLoadMultiObjectTemplateFields(
      subqueryFields[relationshipName],
      childRecordsWithParentId.map(({ childRecord }) => childRecord),
      new Set([lookupField]),
    );
    const sheetName = getExcelSafeSheetName(relationshipName.replace(EXCEL_FORBIDDEN_SHEET_NAME_CHARS, ''), Object.keys(output));

    output[sheetName] = [
      ...getLoadMultiObjectTemplateHeaderRows(childSObject),
      ['Reference Id', `{${lookupField}}`, ...childTemplateFields],
      ...childRecordsWithParentId.map(({ parentId, childRecord }) => [
        childRecord.Id ?? '',
        parentId,
        ...childTemplateFields.map((field) => getLoadMultiObjectTemplateCellValue(childRecord[field])),
      ]),
    ];
  });
