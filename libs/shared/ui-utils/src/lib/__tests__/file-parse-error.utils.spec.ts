...
describe('getFileParseErrorMessage', () => {
  ...
  it('still reports an encrypted workbook renamed to .csv rather than parsing its bytes as text', async () => {
    // Refusing .xls invites the user to rename the file, so the extension must not talk the sniffer out of a workbook
    const error = await parseFile(readRepoFile(ENCRYPTED_FIXTURE_PATH), { extension: '.csv' }).catch((ex) => ex);
