...
async function mergeLocalforageHistoryMap<T extends { lastRun: Date }>(storageKey: string, imported: Record<string, T>): Promise<number> {
  ...
  try {
    const existing = (await getLocalStore().getItem<Record<string, T>>(storageKey)) || {};
...
async function mergeRecentRecords(imported: Record<string, ClientDataExportRecentRecord[]>): Promise<number> {
  ...
  let importedCount = 0;
  try {
    const existing = (await getLocalStore().getItem<Record<string, ClientDataExportRecentRecord[]>>(INDEXED_DB.KEYS.recentRecords)) || {};
...
async function mergeDeployHistory(imported: SalesforceDeployHistoryItem[]): Promise<number> {
  ...
  try {
    const existing = (await getLocalStore().getItem<SalesforceDeployHistoryItem[]>(INDEXED_DB.KEYS.deployHistory)) || [];
