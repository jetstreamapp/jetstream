...
async function getLocalforageMap<T>(key: string): Promise<Record<string, T>> {
  try {
    return (await getLocalStore().getItem<Record<string, T>>(key)) || {};
  } catch (ex) {
...
async function getDeployHistoryMetadata(): Promise<SalesforceDeployHistoryItem[]> {
  try {
    const items = (await getLocalStore().getItem<SalesforceDeployHistoryItem[]>(INDEXED_DB.KEYS.deployHistory)) || [];
