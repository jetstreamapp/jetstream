import { DataHistorySettings, DataHistoryTierLimits } from '@jetstream/types';
 ...
 * user-facing setting.
 *
 * `DataHistoryTierLimits` (the shape) lives in `@jetstream/types` so `ui-app-state` can hold the
 * resolved limits in an atom without depending on this library; the tiers and the rules that pick
 * one stay here.
 */
