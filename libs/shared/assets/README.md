...
Every file the API previously served at `/assets/images/<name>` is still at that exact path, with two exceptions:
`salesforce-logo.svg` and `salesforce-white.svg` moved to `/assets/images/third-party/`. Neither had an in-repo
consumer, so the old URLs now 404 - restore a root-level copy if an external page turns out to link them.
