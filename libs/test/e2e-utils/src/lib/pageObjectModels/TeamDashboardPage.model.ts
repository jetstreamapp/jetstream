...
export interface SeatSummaryExpectation {
  ...
  available: number;
  /** Seats held by pending invitations; they count toward usage in the over-allocated copy */
  reserved?: number;
  /** The cap the footer counts against, which is the pending target while a decrease is scheduled */
  effective?: number;
  /** Manual-billing teams have their cap attributed to the agreement */
 ...
 * so it is the primary assertion for the numbers shown on the dashboard. Mirrors the branches in
 * `getSeatFooterMessage` (libs/features/teams/.../team-seats.utils.ts) exactly.
 */
export function getSeatFooterText({
  purchased,
  used,
  available,
  reserved = 0,
...
}: SeatSummaryExpectation): string {
  const agreementSuffix = hasManualBilling ? ' (set by your agreement)' : '';
  const seatWord = effective === 1 ? 'seat' : 'seats';

  if (available > 0) {
    return `${available} of ${effective} ${seatWord} available${agreementSuffix}.`;
  }
  if (available === 0) {
    return effective === 1 ? `Your 1 seat is in use${agreementSuffix}.` : `All ${effective} seats are in use${agreementSuffix}.`;
  }

  const reservedClause = reserved > 0 ? `, including ${reserved} reserved for pending invitations` : '';
  return `Using ${used + reserved} of ${effective} ${seatWord}${agreementSuffix}${reservedClause} (${Math.abs(available)} over).`;
}
