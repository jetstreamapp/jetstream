...
export async function pruneExpiredRecords() {
  ...
  await prisma.passwordResetToken.deleteMany({
    where: {
      expires: { lte: addDays(startOfDay(new Date()), -3) },
    },
  });
}
...
// FIXME: define type for "type"
export async function getTotpAuthenticationFactor(userId: string) {
  return prisma.authFactors.findFirstOrThrow({
    select: {
      type: true,
      enabled: true,
      secret: true,
...
export async function createOrUpdateOtpAuthFactor(userId: string, secret: string) {
