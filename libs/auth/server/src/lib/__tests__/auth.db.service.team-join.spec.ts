...
function makeSeatLimitError(kind: 'ADD' | 'ACCEPT_INVITATION') {
  return new SeatLimitError({ code: 'NO_SEATS', teamId: TEAM_ID, kind, seats, role: 'MEMBER' });
}
...
describe('domain auto-join for a new user', () => {
  ...
  /** The invite lookup reads the invitation first; the auto-join lookup for the same email comes second. */
  function withNoPendingInvitationForEmail() {
    prismaMock.teamMemberInvitation.findFirst.mockResolvedValueOnce(makeInvite({ autoAddToTeam: true })).mockResolvedValueOnce(null);
  }

  it('skips auto-join for an invited user so the accept step converts their reservation instead of counting it against them', async () => {
    prismaMock.user.create.mockResolvedValue(makeUser(null));

    const result = await signInWithGoogle();

    expect(teamSeatsMock.checkSeatAvailability).not.toHaveBeenCalled();
    expect(prismaMock.teamMemberInvitation.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({ where: { teamId: TEAM_ID, email: EMAIL, expiresAt: { gte: expect.any(Date) } } }),
    );
  ...
  it('checks for a new seat (ADD) under the team lock and attaches the membership when one is free', async () => {
    withNoPendingInvitationForEmail();
    ...
    expect(teamSeatsMock.checkSeatAvailability).toHaveBeenCalledWith(prismaMock, { teamId: TEAM_ID, kind: 'ADD', role: 'MEMBER' });
