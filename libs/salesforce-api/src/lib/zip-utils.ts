// Web-compatible streaming zip implementation
class Crc32 {
  private crc: number;
...
export class ZipUtils {
  static createByteArray(data: Array<{ data: number | bigint | Uint8Array | number[]; size?: number }>): Uint8Array {
    const size = data.reduce((acc, value) => {
      return acc + (value.size ? value.size : (value.data as Uint8Array).length);
    }, 0);
    ...
    let i = 0;
    data.forEach((entry) => {
      if ((entry.data as Uint8Array).length !== undefined) {
        array.set(entry.data as Uint8Array, i);
        i += (entry.data as Uint8Array).length;
      } else {
        const numericData = entry.data as number | bigint;
        switch (entry.size) {
          case 1:
            dataView.setInt8(i, Number(numericData));
            break;
          case 2:
            dataView.setInt16(i, Number(numericData), LITTLE_ENDIAN);
            break;
          case 4:
            dataView.setInt32(i, Number(numericData), LITTLE_ENDIAN);
            break;
          case 8:
            dataView.setBigInt64(i, typeof numericData === 'bigint' ? numericData : BigInt(numericData), LITTLE_ENDIAN);
            break;
          default:
            throw new Error(`No handler defined for data size ${entry.size}`);
        }
        i += entry.size as number;
      }
    });
    return array;
  }
  ...
  /**
   * Calculate the total size of a zip file given the files to be included
   * This allows us to set Content-Length header for proper download progress
   */
  static calculateZipSize(files: Array<{ name: string; size: number }>, useZip64 = false): bigint {
    const localHeaderSize = (file: { name: string; size: number }) => BigInt(30 + file.name.length + (useZip64 ? 32 : 0));
    const dataDescriptorSize = useZip64 ? BigInt(20) : BigInt(12);
    const centralDirectoryHeaderSize = (file: { name: string; size: number }) => BigInt(46 + file.name.length + (useZip64 ? 32 : 0));
    const endOfCentralDirectorySize = BigInt(22);
    const zip64EndOfCentralDirectoryRecordSize = BigInt(56);
    const zip64EndOfCentralDirectoryLocatorSize = BigInt(20);

    let totalSize = files.reduce((acc, file) => {
      return acc + localHeaderSize(file) + BigInt(file.size) + dataDescriptorSize + centralDirectoryHeaderSize(file);
    }, BigInt(0));

    totalSize += endOfCentralDirectorySize;

    if (useZip64) {
      totalSize += zip64EndOfCentralDirectoryRecordSize;
      totalSize += zip64EndOfCentralDirectoryLocatorSize;
    }
...
export class StreamingZip {
  ...
  constructor(zip64 = false) {
    ...
    this.outputStream = new ReadableStream({
      ...
      cancel: () => {
        // Client disconnected or download was canceled
        this.abort();
      },
    });
  }
  ...
  /**
   * Register a callback to be called when the stream is aborted
   * Used to cancel upstream fetch operations
   */
  ...
  /**
   * Abort the zip stream and trigger all abort callbacks
   */
  abort(): void {
    if (this.aborted) {
      return;
  ...
  private async enqueue(data: Uint8Array): Promise<void> {
    if (this.outputController) {
      this.outputController.enqueue(data);
      // Check desired size for backpressure
      // When desiredSize is null or <= 0, the consumer is slow and we should wait
      const desiredSize = this.outputController.desiredSize;
      if (desiredSize !== null && desiredSize <= 0) {
        // Wait for the stream to drain before continuing
        // This prevents memory buildup when the client has a slow connection
  ...
  private getZip64ExtraField(fileSizeBig: bigint, localFileHeaderOffsetBig: bigint): Uint8Array {
  ...
  async startFile(fileName: string): Promise<void> {
    if (this.isWritingFile() || this.finished) {
      throw new Error('Cannot start a new file while another file is being written or zip is finished');
    }
    ...
    const header = ZipUtils.createByteArray([
      ...
      { data: nameBuffer },
      { data: this.zip64 ? this.getZip64ExtraField(BigInt(0), this.byteCounterBig) : new Uint8Array(0) },
    ]);

    await this.enqueue(header);
    this.byteCounterBig += BigInt(header.length);
  }

  async appendData(data: Uint8Array): Promise<void> {
    if (!this.isWritingFile() || this.finished) {
      throw new Error('No file is currently being written');
    }

    await this.enqueue(data);
    this.byteCounterBig += BigInt(data.length);
    const currentFile = this.fileRecord[this.fileRecord.length - 1];
    currentFile.crc.append(data);
    currentFile.sizeBig += BigInt(data.length);
  }
  ...
  async endFile(): Promise<void> {
    ...
    await this.enqueue(dataDescriptor);
  ...
  async finish(): Promise<void> {
    if (this.isWritingFile() || this.finished) {
      throw new Error('Cannot finish zip while a file is being written or zip is already finished');
    }

    let centralDirectorySizeBig = BigInt(0);
    const centralDirectoryStartBig = this.byteCounterBig;

    for (const file of this.fileRecord) {
      ...
      const header = ZipUtils.createByteArray([
        ...
        { data: ZipUtils.getDateStruct(date), size: 2 },
        { data: crc.get(), size: 4 },
        { data: this.zip64 ? 0xffffffff : Number(sizeBig), size: 4 },
        { data: this.zip64 ? 0xffffffff : Number(sizeBig), size: 4 },
        { data: nameBuffer.length, size: 2 },
        { data: this.zip64 ? 32 : 0, size: 2 },
        { data: 0x0000, size: 2 },
        ...
        { data: 0x0000, size: 2 },
        { data: 0x00000000, size: 4 },
        { data: this.zip64 ? 0xffffffff : Number(headerOffsetBig), size: 4 },
        { data: nameBuffer },
    ...
    if (this.zip64) {
      ...
      await this.enqueue(zip64EndOfCentralDirectoryRecord);
      ...
      await this.enqueue(zip64EndOfCentralDirectoryLocator);
    ...
    const endOfCentralDirectoryRecord = ZipUtils.createByteArray([
      ...
      { data: this.zip64 ? 0xffff : this.fileRecord.length, size: 2 },
      { data: this.zip64 ? 0xffffffff : Number(centralDirectorySizeBig), size: 4 },
      { data: this.zip64 ? 0xffffffff : Number(centralDirectoryStartBig), size: 4 },
      { data: 0, size: 2 },
    ]);
    await this.enqueue(endOfCentralDirectoryRecord);
