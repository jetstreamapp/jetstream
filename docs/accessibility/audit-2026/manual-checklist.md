...
workable / P3 = polish). When in doubt, write it down — triage later.
...
| K3  | Can I always get out?                                               | Focus gets stuck in a widget/editor and Tab won't leave (try **Esc** in Monaco editors — this is finding C9) | 2.1.2        |
...
Known-suspect widgets to hit deliberately: **tabs** (arrow keys likely don't switch them — C2),
**time picker** (likely unusable — C3), **query filter drag-and-drop** (is there ANY keyboard way to
reorder? — C8), **comboboxes** (arrow keys, Enter to select, Esc to close), **data grid** (arrow
keys between cells, Enter to edit, Esc to cancel).
...
| S2  | In comboboxes/lists, is the **highlighted option announced** as you arrow through?                                                           | You arrow down and hear nothing (this is finding C1 — expect it to fail)   | 4.1.2        |
...
| Automation control  | Toggle something on/off, review the tab navigation (C2 lives here)                | ☐   | ☐   | ☐   |
| Permissions manager | Select profiles + objects, edit a permission in the grid                          | ☐   | ☐   | ☐   |
| Deploy metadata     | Select metadata, walk to the review step (don't deploy), use date filters (C3)    | ☐   | ☐   | ☐   |
| Anonymous Apex      | Type in the Monaco editor, try to Tab OUT of it, run (C9, K3, S3)                 | ☐   | ☐   | ☐   |
...
```
| M1 | Arrowing through the org combobox announces nothing (S2) | 4.1.2 | P1 | Combobox.tsx | open |
```

Two extra rules:

- **Expected failures**: C1–C10 in the findings log predict several of these checks will fail — when
  they do, just add "confirmed by manual audit" to the existing row rather than a new finding.
- **Passes matter too.** For each flow, note "keyboard pass OK" / "VO pass OK" at the bottom of the
  findings file — the VPAT needs positive evidence for the "Supports" rows, not just a list of bugs.
