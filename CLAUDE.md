...
create/overwrite/unchanged plan and asking for confirmation before writing. Files tracked in either
worktree are skipped unless `--include-tracked` is passed. Symlinks (including parent directories) and
special files are skipped. On POSIX systems, copies also match the source file's permission bits.
