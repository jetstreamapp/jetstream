...
- **Formatting and imports are auto-enforced.** oxfmt (`pnpm format`) and an organize-imports step run on every change. Do NOT comment on whitespace, trailing commas, quote style, line length, or import ordering/grouping.
...
- This repo enables oxlint's `react-hooks` rules (exhaustive-deps, rules-of-hooks). Don't suggest stashing the "latest" handler/value in a mutable ref purely to drop it from a dependency array — it conflicts with our readability preference. Genuine stale-closure bugs are still worth raising.
