...
[#macro head title="Login | Jetstream" author="Jetstream" description="Salesforce Administration Redefined."]
<head>
  <title>${title}</title>
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
  <meta name="application-name" content="Jetstream">
  <meta name="author" content="Jetstream">
  ...
  <script>
    ...
    function closeAlert(event) {
      try {
        ...
        console.warn('There was an error dismissing the alert.', ex);
      }
    }

  function load() {
    document.querySelectorAll('button[title=Close]').forEach(function(btn) {
...
<div id="content" style="height: 100vh;" class="slds-grid slds-grid_vertical-align-center slds-grid_align-center">
  <div class="slds-col">
    <div class="slds-m-bottom_large">
      <img src="/images/jetstream-logo.svg" alt="Jetstream Logo" />
    </div>

    <div class="slds-box slds-theme_default slds-p-around_large" style="min-width: 33rem; max-width: 33rem;">
      [#if title?has_content]
        <div class="slds-text-heading_large slds-text-align_center slds-m-bottom_medium">
          ${title}
        </div>
      [/#if]
    ...
    <div class="slds-form-element">
      <label class="slds-form-element__label" for="locale-select">Language</label>
      <div class="slds-form-element__control">
        <div class="slds-select_container">
          <select id="locale-select" name="locale" class="slds-select">
...
[#macro alert message type icon includeDismissButton=true]
  <div class="slds-notify_container slds-is-relative">
    <div class="slds-notify slds-notify_toast slds-theme_${type}" role="status">
      ...
      <span class="slds-icon_container slds-icon-utility-${type} slds-m-right_small slds-no-flex slds-align-top">
        <svg class="slds-icon slds-icon_small" aria-hidden="true">
          <use xlink:href="/icons/utility-sprite/svg/symbols.svg#${type}"></use>
        </svg>
      </span>
...
[#macro input type name id inlineHelptext="" autocapitalize="none" autocomplete="on" autocorrect="off" autofocus=false spellcheck="false" label="" placeholder="" leftAddon="" required=false tooltip="" disabled=false class="" dateTimeFormat=""]
[#-- Possible bug in CSS class here - if issues, check here first :) --]
<div class="slds-form-element slds-m-bottom_small">
  [#if label?has_content][#t/]
    <label class="slds-form-element__label" for="${id}">${label}[#if required]<abbr class="slds-required" title="required"> *</abbr>[/#if][#t/]
  ...
  [/#if]
  <div class="slds-form-element__control">
    <input id="${id}" class="slds-input ${class}" type="${type}" name="${name}" [#if type != "password"]value="${(name?eval!'')}"[/#if] autocapitalize="${autocapitalize}" autocomplete="${autocomplete}" autocorrect="${autocorrect}" spellcheck="${spellcheck}" [#if autofocus]autofocus="autofocus"[/#if] placeholder="${placeholder}" [#if disabled]disabled="disabled"[/#if]/>
    [#if inlineHelptext?has_content][#t/]
      <div class="slds-form-element__help">${inlineHelptext}</div>
    [/#if][#t/]
  </div>
  [#if dateTimeFormat != ""]
    <input type="hidden" name="${name}@dateTimeFormat" value="${dateTimeFormat}"/>
...
[#macro errors field]
  [#if fieldMessages[field]?has_content]
    <span class="slds-text-color_error">[#list fieldMessages[field] as message]${message?no_esc}[#if message_has_next], [/#if][/#list]</span>
  [/#if]
[/#macro]

[#macro button text icon="" class="" color="blue" disabled=false]
  <button class="slds-button slds-m-top_small ${class}"[#if disabled] disabled=""[/#if]>${text}</button>
[/#macro]

[#macro link url class="" extraParameters=""]
