...
export const upsertTeamWithBillingAccount = async ({
  ...
  if (existingTeam) {
    return prisma.team.update({
      ...
      data: {
        status,
        // The customer is re-pointed as well as created: a team whose Stripe customer was deleted checks
        // out again against a fresh one, and a billing account still bound to the deleted id would keep
        // every later sync reporting CUSTOMER_IS_DELETED and the team past due for good
        billingAccount: existingTeam.team.billingAccount
          ? { update: { customerId: billingAccountCustomerId, manualBilling } }
...
export async function updateTeamMemberRole({
  ...
  const updatedTeamMember = await withTeamSeatLock(teamId, async (tx) => {
    ...
    if (needsSeat) {
      await assertSeatAvailable(tx, { teamId, kind: 'ADD', role: nextRole });
    }
...
export async function createTeamInvitation({
  ...
  return withTeamSeatLock(teamId, async (tx) => {
    ...
    if (isBillableRole(role)) {
      await assertSeatAvailable(tx, { teamId, kind: 'ADD', role });
    }
...
export async function acceptTeamInvitation({ user, teamId, token }: { user: UserProfileSession; teamId: string; token: string }): Promise<{
  ...
  const { role, features } = await withTeamSeatLock(teamId, (tx) =>
    addMemberFromInvitation(tx, { teamId, userId: user.id, invitation: existingInvitation }),
  );
...
export async function revokeTeamInvitation({
  ...
  // Under the team seat lock so an accept that already re-read this invitation (dashboard, signup or SSO)
  // finishes converting it before it can disappear mid-transaction
  await withTeamSeatLock(teamId, async (tx) => {
    const result = await tx.teamMemberInvitation.deleteMany({
      where: { id, teamId, role: expectedRole },
    });
