...
export const routeDefinition = {
  ...
  resendVerification: {
    ...
    validators: {
      body: z.object({ csrfToken: z.string(), type: z.enum(['email', '2fa-email']) }),
      hasSourceOrg: false,
    },
  },
  requestPasswordReset: {
    ...
    validators: {
      body: z.object({ email: z.string(), csrfToken: z.string() }),
      hasSourceOrg: false,
    },
  },
  validatePasswordReset: {
    ...
    validators: {
      body: z.object({ email: z.string().email(), token: z.string(), password: z.string(), csrfToken: z.string() }),
      hasSourceOrg: false,
    },
  },
};
...
const callback = createRoute(routeDefinition.callback.validators, async ({ body, params, query }, req, res, next) => {
  let provider: Provider | null = null;
  try {
    const providers = listProviders();
    ...
    let isNewUser = false;
    const { pkceCodeVerifier, nonce, linkIdentity: linkIdentityCookie, returnUrl } = getCookieConfig(ENV.ENVIRONMENT === 'production');
    ...
    if (req.session.pendingVerification && req.session.pendingVerification?.length > 0) {
...
const verification = createRoute(routeDefinition.verification.validators, async ({ body }, req, res, next) => {
  try {
    if (!req.session.user || !req.session.pendingVerification) {
      throw new InvalidSession();
    }
    const { csrfToken, code, type } = body;
