 ...
 * past due with no self-serve way back. Omitting the id makes Checkout create a fresh customer, which the
 ...
 */
async function resolveUsableTeamCustomerId(customerId: string | undefined): Promise<string | undefined> {
  if (!customerId) {
    return undefined;
  }
  ...
  async ({ user: sessionUser, body }, req, res) => {
    ...
    if (type === 'TEAM') {
      ...
      session = await stripeService.createCheckoutSession({
        ...
        customerId: team ? await resolveUsableTeamCustomerId(team.billingAccount?.customerId) : user.billingAccount?.customerId,
        user,
        type: 'TEAM',
        teamId: team?.id,
        quantity: seats,
