...
describe('security headers', () => {
  ...
  it('uses a nonce + host-allowlist script-src on the shared/landing policy (NOT strict-dynamic)', () => {
    ...
    // blob: must NOT be a script source on any policy — blob: workers resolve via worker-src/child-src
    // instead, so blob: stays out of script-src where it would be a page-level script sink.
    expect(scriptSrc).not.toContain('blob:');
    expect(directives.workerSrc).toContain('blob:');
    expect(directives.childSrc).toContain('blob:');
  });
  ...
  it('uses a strict-dynamic script-src with a host-allowlist fallback on the /app policy', () => {
    ...
    // allowlist rather than a blanket https: scheme source; blob: is never a script source.
