import { ENV, getExceptionLog, logger, prisma, errorTracker } from '@jetstream/api-config';
...
export async function uncaughtErrorHandler(err: any, req: express.Request, res: express.Response, next: express.NextFunction) {
  try {
    ...
    if (res.headersSent) {
      ...
      try {
        errorTracker.warn('Error not handled by error handler, headers already sent', req, {
    ...
    try {
      errorTracker.warn('Exception in error handler', req, {
        context: `route#errorHandler`,
