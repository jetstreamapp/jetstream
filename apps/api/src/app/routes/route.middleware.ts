...
import { AuthError, ExpiredVerificationToken } from '@jetstream/auth/server';
...
export async function checkAuth(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  // FIXME: we should force login with the example user
  if (ENV.EXAMPLE_USER && req.hostname === 'localhost') {
    if (!req.session.user) {
      req.session.user = ENV.EXAMPLE_USER as UserProfileSession;
