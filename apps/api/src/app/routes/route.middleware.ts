...
export const passwordResetEmailRateLimit = createRateLimit(
  'api_password_reset_email',
  {
    ...
    keyGenerator: (req, res) => {
      // Bound the email before it becomes part of the distributed store key: the backing column is
      // VarChar(512) and distributed limiters fail OPEN on a store error, so an oversized email must
      // not be able to overflow the key and slip past the throttle. Valid addresses (RFC 5321 caps at
      // 254) are well within this, so legitimate input is never truncated.
      const email = typeof req.body?.email === 'string' ? req.body.email.trim().toLowerCase().slice(0, 254) : undefined;
      if (email) {
        return `email:${email}`;
      }
