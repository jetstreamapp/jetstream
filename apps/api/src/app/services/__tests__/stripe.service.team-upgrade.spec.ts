...
import type * as StripeSeatsService from '../stripe-seats.service';
...
type StripeSubscription = Parameters<typeof StripeSeatsService.resolveTeamSeatState>[0][number];
type LiveStripeCustomer = Parameters<typeof StripeService.hasCurrentTeamPlanSubscription>[0];
...
const customerWithPlan = (
  lookupKey: string,
  subscriptionOptions?: Parameters<typeof subscriptionWithPrice>[1],
  customerMetadata: Record<string, string> = { userId: 'user_1', type: 'USER' },
) =>
  ({
    id: 'cus_1',
    deleted: undefined,
...
let stripeSeatsService: typeof StripeSeatsService;
...
beforeEach(async () => {
  ...
  stripeSeatsService = await import('../stripe-seats.service');
});
...
describe('personal -> team plan upgrade', () => {
  ...
  it('names the team from the subscription when the subscription webhook is processed before checkout completes', async () => {
    // A brand-new TEAM customer: Stripe drops the null teamId it was created with, and
    // checkout.session.completed has not run yet to attach the team
    const customer = customerWithPlan('TEAM_MONTHLY', { metadata: { teamName: 'Acme Corp' } }, { userId: 'user_1', type: 'TEAM' });

    await stripeService.saveOrUpdateSubscription({ customer, sendWelcomeEmail: false });

    expect(mocks.upsertTeamWithBillingAccount).toHaveBeenCalledWith({
      userId: 'user_1',
      billingAccountCustomerId: 'cus_1',
      name: 'Acme Corp',
    });
    expect(mocks.customersUpdate).toHaveBeenCalledWith('cus_1', { metadata: { userId: 'user_1', teamId: 'team_new', type: 'TEAM' } });
    expect(mocks.updateTeamSubscriptionStateForCustomer).toHaveBeenCalledWith(expect.objectContaining({ teamId: 'team_new' }));
  });

  it('names the team from the subscription when the billing page sync creates it', async () => {
    mocks.customersRetrieve.mockResolvedValue(customerWithPlan('TEAM_MONTHLY', { metadata: { teamName: 'Acme Corp' } }));

    await stripeService.synchronizeStripeWithJetstreamUserIfRequired({ userId: 'user_1' });

    expect(mocks.upsertTeamWithBillingAccount).toHaveBeenCalledWith(expect.objectContaining({ name: 'Acme Corp' }));
  });
...
describe('customer metadata repair', () => {
  it('writes the user id and type back to a customer that is missing its user id', async () => {
    ...
    await stripeService.saveOrUpdateSubscription({ customer: customerWithPlan('PRO_MONTHLY', undefined, {}), sendWelcomeEmail: false });
...
describe('createCheckoutSession', () => {
  ...
  it('sends the purchased seat count with quantity adjustment disabled and the team name in session and subscription metadata', async () => {
    ...
    expect(mocks.checkoutSessionsCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        // The subscription webhook may create the team before the session completes
        subscription_data: { metadata: { teamName: 'Acme Corp' } },
      }),
    );
  });
  ...
  it('defaults personal plans to a single unit with no team name', async () => {
    ...
    expect(mocks.checkoutSessionsCreate).toHaveBeenCalledWith(expect.not.objectContaining({ subscription_data: expect.anything() }));
  });
});

describe('hasCurrentTeamPlanSubscription', () => {
  const customerWithSubscriptions = (...subscriptions: ReturnType<typeof subscriptionWithPrice>[]) =>
    ({ ...customerWithPlan('TEAM_MONTHLY'), subscriptions: { object: 'list', data: subscriptions } }) as unknown as LiveStripeCustomer;

  it.each(['active', 'trialing', 'incomplete', 'past_due', 'unpaid', 'paused'])(
    'counts a %s Team subscription as current so a second checkout is refused',
    (status) => {
      const customer = customerWithSubscriptions({ ...subscriptionWithPrice('TEAM_MONTHLY'), status });

      expect(stripeService.hasCurrentTeamPlanSubscription(customer)).toBe(true);
    },
  );

  it.each(['canceled', 'incomplete_expired'])('lets a team whose subscription is %s subscribe again', (status) => {
    const customer = customerWithSubscriptions({ ...subscriptionWithPrice('TEAM_MONTHLY'), status });

    expect(stripeService.hasCurrentTeamPlanSubscription(customer)).toBe(false);
  });

  it('ignores personal plan subscriptions', () => {
    expect(stripeService.hasCurrentTeamPlanSubscription(customerWithSubscriptions(subscriptionWithPrice('PRO_MONTHLY')))).toBe(false);
  });

  it('returns false for a customer with no subscriptions', () => {
    expect(stripeService.hasCurrentTeamPlanSubscription(customerWithSubscriptions())).toBe(false);
  });
});
...
describe('resolveTeamSeatState', () => {
  it('returns null when the customer has no team item', async () => {
    expect(await stripeSeatsService.resolveTeamSeatState(asSubscriptions(subscriptionWithPrice('PRO_MONTHLY')))).toBeNull();
  ...
  it('resolves a current volume price: no included seats, quantity is the seat count', async () => {
    ...
    const state = await stripeSeatsService.resolveTeamSeatState(
  ...
  it('fails the sync when a tiered price comes back without a tier table', async () => {
    mocks.pricesRetrieve.mockResolvedValue({ id: 'price_TEAM_MONTHLY', tiers: undefined });

    await expect(
      stripeSeatsService.resolveTeamSeatState(
        asSubscriptions(subscriptionWithPrice('TEAM_MONTHLY', { quantity: 1, billingScheme: 'tiered' })),
      ),
    ).rejects.toThrow(/no tier table/);
  });

  it('skips the tier lookup for per-unit prices', async () => {
    const state = await stripeSeatsService.resolveTeamSeatState(asSubscriptions(subscriptionWithPrice('TEAM_MONTHLY', { quantity: 3 })));
...
describe('previewTeamSeatChange', () => {
  it('previews the recurring total without prorations and the amount due now with them', async () => {
    ...
    const result = await stripeSeatsService.previewTeamSeatChange({
      customerId: 'cus_1',
...
describe('commitTeamSeatIncrease', () => {
  it('updates the item with an immediate prorated invoice that fails atomically on declined payment', async () => {
    await stripeSeatsService.commitTeamSeatIncrease({ subscriptionItemId: 'si_1', quantity: 8, prorationDate: 1_760_500_000 });
...
describe('releaseTeamSeatSchedule', () => {
  it('releases the schedule', async () => {
    await stripeSeatsService.releaseTeamSeatSchedule('sub_sched_1');
  ...
  it('treats an already-released schedule (invalid request) as done', async () => {
    ...
    await expect(stripeSeatsService.releaseTeamSeatSchedule('sub_sched_1')).resolves.toBeUndefined();
  });

  it('rethrows other failures', async () => {
    mocks.schedulesRelease.mockRejectedValueOnce(new Error('network'));

    await expect(stripeSeatsService.releaseTeamSeatSchedule('sub_sched_1')).rejects.toThrow('network');
  });
});
...
describe('fetchLatestInvoiceForSubscription', () => {
  it('returns the newest invoice on the subscription in dollars', async () => {
    ...
    const invoice = await stripeSeatsService.fetchLatestInvoiceForSubscription('sub_1');
  ...
  it('returns null when the subscription has no invoice yet', async () => {
    ...
    expect(await stripeSeatsService.fetchLatestInvoiceForSubscription('sub_1')).toBeNull();
  });
});
