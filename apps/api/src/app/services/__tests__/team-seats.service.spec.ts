...
const mocks = vi.hoisted(() => ({
  ...
  reservePendingSeatDecrease: vi.fn(async () => ({})),
  clearPendingSeatDecrease: vi.fn(async () => ({})),
  clearSeatScheduleId: vi.fn(async () => ({})),
...
vi.mock('../../db/subscription.db', () => ({
  ...
  reservePendingSeatDecrease: mocks.reservePendingSeatDecrease,
  clearPendingSeatDecrease: mocks.clearPendingSeatDecrease,
  clearSeatScheduleId: mocks.clearSeatScheduleId,
}));
...
describe('previewSeatChange rejections', () => {
  ...
  it('rejects teams without a billing account as having no active subscription', async () => {
    ...
    await expectSeatError(service.previewSeatChange({ teamId: 'team_1', seats: 8 }), 'SEATS_NO_SUBSCRIPTION', /active subscription/);
  });
...
describe('commitSeatChange INCREASE', () => {
  ...
  it('releases a pending decrease before the update and records the cancellation', async () => {
    ...
    // Only the stale schedule pointer is dropped before Stripe is called: the reserved lower cap stays
    // enforced until the increase has landed, and the re-sync afterwards is what clears it
    expect(mocks.clearSeatScheduleId).toHaveBeenCalledWith({ teamId: 'team_1' });
    expect(mocks.clearPendingSeatDecrease).not.toHaveBeenCalled();
...
describe('commitSeatChange DECREASE', () => {
  it('schedules the decrease for period end, persists the pending fields and records it', async () => {
    ...
    // The target is reserved under the lock before Stripe is asked, then completed with the schedule id
    expect(mocks.reservePendingSeatDecrease).toHaveBeenCalledWith(prismaMock, {
      teamId: 'team_1',
      quantity: 4,
      effectiveAt: fromUnixTime(PERIOD_END),
    });
    expect(mocks.reservePendingSeatDecrease.mock.invocationCallOrder[0]).toBeLessThan(
  ...
  it('replaces a pending decrease with a different target by releasing the old schedule first', async () => {
    ...
    // Replacing a decrease must drop only the stale schedule pointer: clearing the reserved count would
    // put the cap back to the purchased total across both Stripe calls, letting an invite take a seat
    // the new count cannot cover
    expect(mocks.clearSeatScheduleId).toHaveBeenCalledWith({ teamId: 'team_1' });
    expect(mocks.clearPendingSeatDecrease).not.toHaveBeenCalled();
    expect(mocks.reservePendingSeatDecrease).toHaveBeenCalledWith(prismaMock, expect.objectContaining({ teamId: 'team_1', quantity: 4 }));
  });
});
...
describe('assertCheckoutSeatsCoverUsage', () => {
  it('rejects a purchase below active members plus pending invitations', async () => {
    mocks.getTeamSeatSummary.mockResolvedValue(await usage(3, 1));

    const error = await service.assertCheckoutSeatsCoverUsage({ teamId: 'team_1', seats: 3 }).catch((ex) => ex);

    expect(error.message).toBe('Your team needs at least 4 seats: 3 in use and 1 reserved by pending invitations.');
    expect(error.additionalData).toEqual({ code: 'SEATS_BELOW_MINIMUM', minimum: 4, used: 3, reserved: 1 });
    expect(mocks.getTeamSeatSummary).toHaveBeenCalledWith(prismaMock, { teamId: 'team_1' });
  });

  it('accepts a purchase that covers usage without loading any Stripe seat state', async () => {
    mocks.getTeamSeatSummary.mockResolvedValue(await usage(3, 1));

    await expect(service.assertCheckoutSeatsCoverUsage({ teamId: 'team_1', seats: 4 })).resolves.toBeUndefined();
    expect(mocks.getTeamBillingAccountForSeats).not.toHaveBeenCalled();
    expect(mocks.resolveTeamSeatState).not.toHaveBeenCalled();
  });

  it('accepts a single seat for a team with no members or invitations holding one', async () => {
    mocks.getTeamSeatSummary.mockResolvedValue(await usage(0));

    await expect(service.assertCheckoutSeatsCoverUsage({ teamId: 'team_1', seats: 1 })).resolves.toBeUndefined();
  });
});
