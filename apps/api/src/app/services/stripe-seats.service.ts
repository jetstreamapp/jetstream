...
export async function fetchPriceTiers(priceId: string): Promise<JetstreamPriceTier[] | null> {
  ...
  // Only tiered prices are looked up, so a missing or empty table is an anomaly worth retrying on the
  // next call rather than remembering for the cache lifetime
  if (tiers?.length) {
...
type PhaseSettingsParams = Pick<
  Stripe.SubscriptionScheduleUpdateParams.Phase,
  'default_payment_method' | 'default_tax_rates' | 'metadata' | 'collection_method' | 'invoice_settings'
>;

/**
 * Subscription settings Stripe may have copied onto the phase, each included only when set, so an
 * empty phase leaves the update params unchanged.
 */
function toPhaseSettingsParams(phase: Stripe.SubscriptionSchedule.Phase): PhaseSettingsParams {
  const settings: PhaseSettingsParams = {};
  const defaultPaymentMethodId = getStripeObjectId(phase.default_payment_method);
  if (defaultPaymentMethodId) {
    settings.default_payment_method = defaultPaymentMethodId;
  }
  if (phase.default_tax_rates?.length) {
    settings.default_tax_rates = phase.default_tax_rates.map(({ id }) => id);
  }
  if (phase.metadata && Object.keys(phase.metadata).length > 0) {
    settings.metadata = phase.metadata;
  }
  if (phase.collection_method) {
    settings.collection_method = phase.collection_method;
    // A send_invoice phase needs its payment terms
    const daysUntilDue = phase.invoice_settings?.days_until_due;
    if (typeof daysUntilDue === 'number') {
      settings.invoice_settings = { days_until_due: daysUntilDue };
    }
  }
  return settings;
}
 ...
 * The payment method, tax rates, metadata and collection settings on the phase are re-sent too: an
 * update unsets every phase field it omits, and updating the current phase updates the subscription, so
 * dropping them could leave the renewal invoice with no card to charge.
 */
export function buildSeatDecreaseScheduleParams({
  ...
  const settingsParams = toPhaseSettingsParams(currentPhase);
  return {
