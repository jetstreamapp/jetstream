...
function getSeatFloor(context: SeatContext, usage: TeamSeatSummary): number {
  return getMinimumSeats({ used: usage.used, reserved: usage.reserved, includedSeats: context.seatState.includedSeats });
}
...
export async function previewSeatChange({ teamId, seats }: { teamId: string; seats: number }): Promise<TeamSeatChangePreview> {
  ...
  const minimumSeats = getSeatFloor(context, usage);
 ...
 *
 * `keepReservation` leaves the reserved seat count untouched for a caller that is about to write a new
 * decrease. Clearing it there would drop the enforced cap back to the purchased count for the duration
 * of the Stripe calls, letting a concurrent invite take a seat the replacement count cannot cover.
 */
...
export async function commitSeatChange({
  ...
  await withTeamSeatLock(teamId, async (tx) => {
    ...
    const validation = validateSeatRequest({ context, usage, requested: seats });
    if (!validation.ok) {
      throwSeatRejection(validation, { context, usage, minimumSeats: getSeatFloor(context, usage) });
    }
