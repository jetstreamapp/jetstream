...
export async function updateLoginConfiguration({
  ...
  const sessionsRevoked = await teamDbService
    .revokeSessionThatViolateLoginConfiguration({ teamId, skipUserIds: [runningUserId] })
    .catch((ex) => {
      logger.error({ teamId, error: getErrorMessage(ex) }, 'Error revoking sessions that violate updated login configuration');
      return 0;
    });

  return { team: result.team, previousLoginConfig: result.previousLoginConfig, sessionsRevoked };
}
...
export async function createInvitation({
  ...
  await sendTeamInviteEmail({
    emailAddress: createdInvitation.email,
    teamId,
    teamName: createdInvitation.team.name,
  ...
  return {
    ...
    createdInvitation: {
      ...
      features: createdInvitation.features as string[],
...
export async function resendInvitation({
  ...
  await sendTeamInviteEmail({
    emailAddress: updatedInvitation.email,
    teamId,
...
export async function verifyTeamInvitation({
  ...
  user: UserProfileSession;
  currentSessionProvider: OauthProviderType | SsoProviderType | 'credentials';
  teamId: string;
  token: string;
}) {
...
export async function acceptTeamInvitation({
  ...
  teamId: string;
  token: string;
}): Promise<{ email: string; role: string; features: string[] }> {
