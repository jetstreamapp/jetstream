...
function createCustomer({
  ...
  subscriptions?: Array<{
    ...
    items: Array<{ id: string; priceId: string; lookupKey: string | null; quantity?: number; billingScheme?: 'tiered' | 'per_unit' }>;
  }>;
}) {
  return {
    id,
      ...
      data: subscriptions.map((subscription) => ({
        ...
        items: {
          data: subscription.items.map((item) => ({
            ...
            // Team prices are tiered unless a test says otherwise, so the tier table is looked up by default
            price: { id: item.priceId, lookup_key: item.lookupKey, billing_scheme: item.billingScheme ?? 'tiered' },
          })),
        },
      })),
    },
...
describe('backfillTeamSeats', () => {
  ...
  it('counts a tiered price without a tier table as a failure rather than shrinking the cap', async () => {
    const harness = createHarness({
      accounts: [createAccount({ teamId: 'team-legacy', customerId: 'cus_legacy', licenseCountLimit: 5, includedSeats: 5 })],
      customersById: {
        cus_legacy: createCustomer({
          id: 'cus_legacy',
          subscriptions: [
      ...
      // Stripe answered without the expanded tier table
      tiersByPriceId: {},
    });

    const result = await backfillTeamSeats({ ...harness, dryRun: false, now: NOW });

    expect(result).toEqual({ dryRun: false, scanned: 1, updated: 0, skipped: 0, warnings: 0, failures: 1 });
    expect(harness.update).not.toHaveBeenCalled();
    expect(harness.logger.error).toHaveBeenCalledWith(
      expect.objectContaining({ teamId: 'team-legacy', message: expect.stringContaining('no tier table') }),
  ...
  it('skips the tier lookup for per-unit prices', async () => {
    const harness = createHarness({
      accounts: [createAccount({ teamId: 'team-unit', customerId: 'cus_unit' })],
      customersById: {
        cus_unit: createCustomer({
          id: 'cus_unit',
          subscriptions: [
            {
              id: 'sub_unit',
              status: 'active',
              items: [{ id: 'si_unit', priceId: 'price_unit', lookupKey: 'TEAM_MONTHLY', quantity: 3, billingScheme: 'per_unit' }],
            },
          ],
        }),
      },
    ...
    expect(result).toEqual({ dryRun: false, scanned: 1, updated: 1, skipped: 0, warnings: 0, failures: 0 });
    expect(harness.pricesRetrieve).not.toHaveBeenCalled();
    expect(harness.update).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { teamId: 'team-unit' },
        data: expect.objectContaining({ includedSeats: 0, licenseCountLimit: 3 }),
      }),
    );
  });
