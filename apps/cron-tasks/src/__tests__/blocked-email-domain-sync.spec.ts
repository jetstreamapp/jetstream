...
describe('syncBlockedEmailDomains', () => {
  ...
  it('should abort without writing when a run would remove more domains than a real delisting ever does', async () => {
    // Clears the size floor, but is missing every one of 501 previously synced domains.
    mockFetchResponse(buildList());
    const prisma = mockPrisma(Array.from({ length: 501 }, (_, i) => ({ domain: `delisted-${i}.example`, source: 'LIST_SYNC' as const })));

    await expect(syncBlockedEmailDomains(asPrismaClient(prisma))).rejects.toThrow(/would remove 501 domains/);
    expect(prisma.blockedEmailDomain.createMany).not.toHaveBeenCalled();
    expect(prisma.blockedEmailDomain.deleteMany).not.toHaveBeenCalled();
  });
