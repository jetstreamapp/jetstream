 ...
 * roughly 9,000 domains and is additive far more often than it is subtractive.
 */
const DEFAULT_LIST_URL =
  'https://raw.githubusercontent.com/disposable-email-domains/disposable-email-domains/main/disposable_email_blocklist.conf';

/**
 * A short fetch is far more likely to be a broken response or an upstream repo change than a genuine
 * mass-delisting, and acting on one would silently drop most of the blocklist. Kept a little over
 * half the list's real size so ordinary churn never trips it - which means a list missing a smaller
 * chunk still clears it, and MAX_REMOVALS_PER_RUN is what catches that. Raise this if the upstream
 * list grows substantially.
 */
const MIN_EXPECTED_DOMAINS = 5000;

/**
 * Upstream delists a handful of domains in a typical week, so a run that would delete more than this
 * is far more likely to be acting on an incomplete list than on a genuine cleanup. Deletions are the
 * only lossy part of the sync, so they get their own ceiling. Raise this deliberately if upstream
 * ever does a large prune.
 */
const MAX_REMOVALS_PER_RUN = 500;
...
export async function syncBlockedEmailDomains(prisma: PrismaClient): Promise<BlockedEmailDomainSyncResult> {
  ...
  if (domainsToRemove.length > MAX_REMOVALS_PER_RUN) {
    throw new Error(
      `Blocklist sync would remove ${domainsToRemove.length} domains (limit ${MAX_REMOVALS_PER_RUN}) - aborting, the fetched list looks incomplete`,
    );
  }
