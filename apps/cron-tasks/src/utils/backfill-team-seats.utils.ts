...
import { BILLABLE_ROLES, JetstreamPriceTier, TEAM_MEMBER_STATUS_ACTIVE } from '@jetstream/types';
...
function createPriceTiersLoader(stripe: SeatBackfillStripeClient) {
  const tiersByPriceId = new Map<string, Promise<JetstreamPriceTier[] | null>>();
  return (priceId: string): Promise<JetstreamPriceTier[] | null> => {
    let pending = tiersByPriceId.get(priceId);
    if (!pending) {
      pending = stripe.prices.retrieve(priceId, { expand: ['tiers'] }).then(({ tiers }) => convertStripePriceTiers(tiers));
...
async function backfillAccount({
  ...
  // Matches the API: only tiered prices carry a tier table, and an unreadable table must fail the team
  // rather than write includedSeats=0, which would shrink a legacy plan's cap to its raw quantity
  const tiers = item.price.billing_scheme === 'tiered' ? await loadPriceTiers(item.price.id) : null;
  if (item.price.billing_scheme === 'tiered' && !tiers?.length) {
    throw new Error(`Tiered price ${item.price.id} returned no tier table, the seat cap cannot be derived`);
  }
  const includedSeats = getIncludedSeatsFromPriceTiers(tiers);
  const desiredSeats = getPurchasedSeatCount({ quantity, includedSeats });
  ...
  // Every column this backfill writes is compared, not just the cap: a rerun after a renewal or after a
  // corrected tier lookup must not report a row as synced while leaving those fields stale. Pending
  // decrease fields are deliberately absent because the backfill never writes them (see the docblock)
