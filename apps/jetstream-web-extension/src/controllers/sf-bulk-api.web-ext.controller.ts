...
const createJob = createRoute(routeDefinition.createJob.validators, async ({ body, jetstreamConn }, _req) => {
  try {
    const options = body;
...
const getJob = createRoute(routeDefinition.getJob.validators, async ({ params, jetstreamConn }, _req) => {
  try {
...
const closeOrAbortJob = createRoute(routeDefinition.closeOrAbortJob.validators, async ({ params, jetstreamConn }, _req) => {
  try {
    const jobId = params.jobId;
...
const addBatchToJob = createRoute(routeDefinition.addBatchToJob.validators, async ({ body, params, query, jetstreamConn }, _req) => {
  try {
    const jobId = params.jobId;
    const csv = body;
...
const addBatchToJobWithBinaryAttachment = createRoute(
  ...
  async ({ body, params, query, jetstreamConn }, _req) => {
    try {
      const jobId = params.jobId;
      const zip = body;
      const closeJob = query.closeJob;
 ...
 *
 */
const downloadResultsFile = createRoute(routeDefinition.downloadResultsFile.validators, async ({ params, query, jetstreamConn }, _req) => {
  try {
    const jobId = params.jobId;
 ...
 */
const downloadResults = createRoute(routeDefinition.downloadResults.validators, async ({ params, query, jetstreamConn }, _req) => {
  try {
    const jobId = params.jobId;
    const batchId = params.batchId;
    const type = query.type;
