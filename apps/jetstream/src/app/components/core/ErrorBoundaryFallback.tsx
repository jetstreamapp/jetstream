...
export const ErrorBoundaryFallback: FunctionComponent<FallbackProps> = ({ error, /** componentStack, */ resetErrorBoundary }) => {
  ...
  const [isFromUpdate, setIsFromUpdate] = useState(() => chunkLoadingRegex.test(error.message));
  ...
  return (
    <Fragment>
      ...
      {isFromUpdate && (
        <div>There is a new version of Jetstream available. Your browser will refresh to make sure you are using the latest version.</div>
      )}
    </Fragment>
  );
};
