...
import { getErrorMessage, getMinimumSeats } from '@jetstream/shared/utils';
import {
...
export const Billing = () => {
  ...
  const fetchSubscriptions = useCallback(async () => {
    ...
    try {
      ...
      setTeam(teamData);
      const minimumSeats = teamData
        ? getMinimumSeats({ used: teamData.seats.used, reserved: teamData.seats.reserved, includedSeats: null })
        : 1;
      setSeats((currentSeats) => Math.max(currentSeats, minimumSeats));
    } catch (ex) {
  ...
  // An existing team must keep every seat it uses or has promised to an invitee
  const minSeats = team ? getMinimumSeats({ used: team.seats.used, reserved: team.seats.reserved, includedSeats: null }) : 1;
  ...
  const handleCheckoutSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      // The server explains its own rejections (seat minimum, manual billing, permissions); only an
      // unexplained failure gets the generic fallback
      setCheckoutSessionError(
        getErrorMessage(ex) || 'There was an error initiating your checkout session, please contact support for assistance.',
      );
    } finally {
  ...
  // A team member without a billing role can buy nothing here: the individual plan is closed to them and
  // team checkout is refused server-side, so the plan picker would only ever dead-end on a permission error
  const isTeamMemberWithoutBillingAccess = !!teamId && !canReadTeam;
  // Manual-billing seats are set by agreement and checkout is refused server-side, so the picker is
  // replaced with the same explanation the team dashboard gives
  const isManagedByAgreement = !!teamId && hasManualBilling;
  const hidesPlanPicker = isTeamMemberWithoutBillingAccess || isManagedByAgreement;
      ...
      <AutoFullHeightContainer className="slds-p-around_small slds-scrollable_none" bufferIfNotRendered={HEIGHT_BUFFER}>
        ...
        {!loading && (
          <>
            ...
            {(!customerWithSubscriptions || !subscriptionStatus.hasActiveSubscriptions) && !hidesPlanPicker && (
            ...
            <div className="slds-p-horizontal_small">
              {customerWithSubscriptions && subscriptionStatus.hasActiveSubscriptions ? (
                <BillingExistingSubscriptions
                  ...
                  teamId={team?.id ?? null}
                />
              ) : isTeamMemberWithoutBillingAccess ? (
                <div data-testid="team-billing-managed-by-admin">
                  <ScopedNotification theme="info">
                    Your subscription is managed by your team. Contact a team admin to change your team's plan or seat count.
                  </ScopedNotification>
                </div>
              ) : isManagedByAgreement ? (
                <div data-testid="team-billing-managed-by-agreement">
                  <ScopedNotification theme="info">
                    Your team's seat limit is set by your billing agreement. Contact support to change your seat count or plan.
                  </ScopedNotification>
                </div>
              ) : (
