...
import { describeSubscriptionItemPricing, formatSeatDate, formatUsd, getIntervalLabel } from './billing.utils';
      ...
      <div className="slds-m-top_x-small slds-text-align_left">
        ...
        {seats.isOverAllocated && (
          <ScopedNotification theme="warning" className="slds-m-top_x-small">
            {hasPendingDecrease
              ? `Your team uses more seats than it will have after the scheduled decrease (${seatsOver} ${pluralizeFromNumber(
                  'seat',
                  seatsOver,
                )} over).`
              : `Your team is using more seats than it has purchased (${seatsOver} ${pluralizeFromNumber('seat', seatsOver)} over).`}{' '}
            {hasManualBilling
              ? 'Deactivate members or contact support to raise your seat limit to avoid interruption.'
              : 'Buy more seats or deactivate members to avoid interruption.'}
          </ScopedNotification>
        )}
        {teamId && (
