...
import { PRECACHEABLE_ASSET_PATTERN } from '../../vite.plugins';
 ...
 * quota cleanup, activate-time cache retention, and the fetch interception rules.
 */
...
function createHarness({
    ...
    return {
      ...
      addAll: async (paths: string[]) => {
        for (const path of paths) {
          ...
          if (exceedQuota) {
            throw new MockQuotaExceededError('quota');
          }
          store.set(path, response);
        }
      },
  ...
  const source = templateSource
    ...
    .replaceAll('__PRECACHE_MANIFEST__', JSON.stringify(manifest))
...
describe('sw.template.js', () => {
  ...
  describe('install', () => {
    ...
    /**
     * Retention only runs in activate, and a worker that waits for every client to close never
     * activates for someone who keeps tabs open for days - every deploy would add another full
     * precache that nothing prunes.
     */
    it('skips waiting so a new worker always reaches activate', async () => {
      const harness = createHarness({ version: 'v1', manifest: ['/index-AAA.js'] });
    ...
    it('gives back every precache when the origin runs out of quota', async () => {
      const harness = createHarness({ version: 'v2', manifest: ['/src-NEW.js'], exceedQuota: true });
      await harness.seedCache(`${SW_PRECACHE_PREFIX}v1`, ['/a.js'], 1);

      await expect(harness.runInstall()).rejects.toThrow('quota');
      expect([...harness.cacheStores.keys()]).toEqual([]);
    });
  });
  ...
  describe('fetch', () => {
    ...
    /**
     * The worker's ASSET_PATTERN is injected from the same PRECACHEABLE_ASSET_PATTERN the plugin uses
     * to build the manifest, so what gets precached and what gets served can never disagree. This
     * asserts the injection is actually wired up rather than re-stating the pattern.
     */
    it.each([
      'index-AAA.js',
      'src-BBB.css',
      ...
      'image-GGG.webp',
      'nested/chunk-HHH.js',
    ])('intercepts %s only when the build would have precached it', async (fileName) => {
      const harness = await setup();
      const result = await harness.dispatchFetch({ url: `${ORIGIN}/${fileName}`, method: 'GET', mode: 'no-cors' });
      expect(result.intercepted).toBe(PRECACHEABLE_ASSET_PATTERN.test(fileName));
    });
