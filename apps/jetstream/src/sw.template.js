 ...
 * the version, precache-manifest and asset-pattern placeholders below at build time and emits the result
 * as `sw.js`, which the API serves at /app/sw.js with `Service-Worker-Allowed: /app`.
 *
 ...
 * - INSTALL SKIPS WAITING. Not an optimization: retention is enforced in `activate`, and a worker that
 *   waits for every client to close never activates for the users who keep tabs open for days - each
 *   deploy would then add another full precache that nothing ever prunes. Taking over immediately is
 *   safe precisely because of the constraints above: assets are content-hashed and requested by exact
 *   URL, so a page loaded from an older build gets the same bytes from a retained cache either way.
 */

const SW_VERSION = '__SW_VERSION__';
const PRECACHE_MANIFEST = __PRECACHE_MANIFEST__;
// Injected from PRECACHEABLE_ASSET_PATTERN in vite.plugins.ts, which builds the manifest above, so the
// worker cannot classify assets differently than the build that chose them. Matched against the
// pathname without its leading slash; `[^/]+` keeps it to the origin root.
const ASSET_PATTERN = /__ASSET_PATTERN__/;
...
/** Every precache, newest first. A cache with no readable meta sorts oldest and is pruned first. */
async function getCachesNewestFirst() {
  ...
  return cachesWithAge.sort((cacheA, cacheB) => cacheB.createdAt - cacheA.createdAt);
}

async function deletePrecaches(cacheNames) {
  ...
  event.waitUntil(
    (async () => {
      ...
      try {
        ...
        // Cache Storage shares the origin's storage quota with IndexedDB (Dexie, localforage) and OPFS.
        // Leaving a half-written precache behind under quota pressure would keep taking space from user
        // data for a worker that can never activate, so give it all back and let the app run without a
        // precache. Still rethrown: install must fail so the previous worker stays in charge.
        if (error && error.name === 'QuotaExceededError') {
          await deletePrecaches((await caches.keys()).filter((cacheName) => cacheName.startsWith(CACHE_PREFIX)));
        }
        throw error;
      }
      // See INSTALL SKIPS WAITING above
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      const expiredCaches = (await getCachesNewestFirst()).slice(MAX_RETAINED_CACHES).filter(({ cacheName }) => cacheName !== CACHE_NAME);
      await deletePrecaches(expiredCaches.map(({ cacheName }) => cacheName));
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin !== self.location.origin || url.pathname === CACHE_META_PATH || !ASSET_PATTERN.test(url.pathname.slice(1))) {
    return;
  }
