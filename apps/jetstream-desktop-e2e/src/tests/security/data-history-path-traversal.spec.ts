...
test.describe('Data History IPC — path traversal', () => {
  ...
  test('rejects a multi-segment or traversal scopeDir on init', async ({ electronApiClient }) => {
    // `..` fails the per-segment allowlist, while `a/b` passes it and is caught by the scope check
    // that requires exactly one segment — assert each specific message so a broken IPC surface
    // (which rejects with `window.electronAPI is not available on this page`) cannot pass this test.
