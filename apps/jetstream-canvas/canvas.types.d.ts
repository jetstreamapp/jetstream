...
declare global {
  ...
  var sr: SfdcCanvasSignedRequest;
  interface Window {
    // Canvas SDK global
    Sfdc: {
      canvas: SfdcCanvas;
  ...
  interface SfdcCanvas extends SfdcCanvasCore, SfdcCanvasModules {}
}

type SfdcCanvasSignedRequestAlgorithm = 'HMACSHA256' | string;

interface SfdcCanvasSignedRequest {
...
interface SfdcCanvasSignedRequestClient {
...
interface SfdcCanvasSignedRequestContext {
...
interface SfdcCanvasSignedRequestUser {
  userId: string;
  userName: string;
...
interface SfdcCanvasSignedRequestLinks {
  loginUrl: string;
...
interface SfdcCanvasSignedRequestApplication {
  name: string;
  canvasUrl: string;
...
interface SfdcCanvasSignedRequestOrganization {
...
interface SfdcCanvasSignedRequestEnvironment {
  referer: string | null;
  locationUrl: string;
...
interface SfdcCanvasSignedRequestDimensions {
  width: string;
  height: string;
  maxWidth: string;
  maxHeight: string;
...
interface SfdcCanvasOAuthClient {
...
interface SfdcCanvasOAuthLoginContext {
...
interface SfdcCanvasClientAjaxSettings {
...
interface SfdcCanvasClientResponse<TPayload = any> {
  seq?: number;
  status: number;
  statusText: string;
...
interface SfdcCanvasClientVersion {
  clientVersion: string; // '41.0' in snippet
  parentVersion: string | undefined;
}

interface SfdcCanvasClientFrameSize {
...
interface SfdcCanvasClientResizeSize {
  height?: string;
  width?: string;
}

interface SfdcCanvasClientEventSubscription {
...
interface SfdcCanvasClientPublishEvent {
  name: string;
  params?: Record<string, any>;
  payload?: any;
}
