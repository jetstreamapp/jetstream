...
const OrgSelectionRequired = ({ children }: { children: React.ReactNode }) => (
  <OrgSelectionRequiredExternal>{children}</OrgSelectionRequiredExternal>
);

const platformEventSubscribeFn: SubscribeOverrideFn = (setMessagesByChannel, onEvent) => {
  return async (channel, replayId) => {
    const callbackFn = onEvent(replayId);
    const onData = (payload: PlatformEventMessage) => {
      logger.log('[CANVAS][PLATFORM EVENT][RECEIVED]', payload);
      callbackFn(payload);
    };
    const onComplete = (payload: { success: true; handle: string[] } | { success: false; errorMessage: string }) => {
      logger.log('[CANVAS][PLATFORM EVENT][SUBSCRIBE]', payload);
    };
    ...
    window.Sfdc.canvas.client.subscribe(window.sr.client, {
      name: 'sfdc.streamingapi',
      params: { topic: channel },
      onData,
      onComplete,
    });
  };
...
const platformEventUnsubscribeFn: UnsubscribeOverrideFn = (setMessagesByChannel) => {
  return async (channel) => {
    window.Sfdc.canvas.client.unsubscribe(window.sr.client, { name: 'sfdc.streamingapi', params: { topic: channel } });
    setMessagesByChannel((item) => {
...
export const AppRoutes = () => {
  ...
  return (
    <Routes>
      <Route path={APP_ROUTES.HOME.ROUTE} element={<AppHome showAlternativeAppFormats={false} hideConnectedAppBanner />} />
      <Route
      ...
      <Route
        ...
        element={
          <OrgSelectionRequired>
            <PlatformEventMonitor subscribeOverrideFn={platformEventSubscribeFn} unsubscribeOverrideFn={platformEventUnsubscribeFn} />
          </OrgSelectionRequired>
        }
      />
      <Route
