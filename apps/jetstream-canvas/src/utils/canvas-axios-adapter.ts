...
import { extensionRoutes } from '../controllers/canvas.routes';
import { initApiClient } from './api-client';
import { getCanvasOrg } from './canvas.utils';

export async function canvasAppAxiosAdapter(config: InternalAxiosRequestConfig): Promise<AxiosResponse> {
...
function getUrl(config: InternalAxiosRequestConfig) {
  ...
  const url = `${window.location.origin}${config.url || '/'}${qs ? `?${qs}` : ''}`;
  return url;
}
