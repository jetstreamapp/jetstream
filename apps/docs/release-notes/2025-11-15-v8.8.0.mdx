...
Salesforce orgs that have not been used in 90 days now enter an expiration workflow. Jetstream sends a consolidated email covering your expiring and expired orgs so nothing disappears without warning.

### Better Organization page

The Organization page now highlights expiring and expired orgs. Reconnecting pre-fills the connection details you already had, so you rarely have to re-enter anything.
...
The old term collided with "Salesforce organization." Everything in the app and docs has been renamed to "Organization Group." Existing data is unaffected; it is purely a naming change.

### Remembered download format

Your last-used export format is now saved as your default, instead of falling back to the same built-in choice every time.
