...
- **Skip records** ignores the first records that meet your criteria, which allows you to work through a large data volume in chunks.

The number of impacted records shown when you validate takes both of these into account, so you will always see exactly how many records will be updated.

:::note

Salesforce does not allow skipping more than 2,000 records in a single query.

If your criteria targets records that no longer match after they are updated, for example **Only if blank**, then you do not need to skip records at all - just run the update again to process the next set of records.
