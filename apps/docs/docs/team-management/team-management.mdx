...
There is no fixed limit on the Team plan. Pricing is per active user, with a lower per-user rate once your team has 6 or more seats.

If you are billed using a purchase order, your license count is set by your agreement, so please contact support to increase it. Otherwise your license count will be based on the number of users that are active and will be prorated as you add and remove users.
