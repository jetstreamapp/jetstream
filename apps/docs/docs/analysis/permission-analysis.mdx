...
description: Scan every profile and permission set in your org for over-access, inconsistent field security, and dead configuration, then export the evidence.
keywords:
  [
    salesforce,
    salesforce admin,
...
- **Cleanup after a migration or merge.** Surface permission sets with no assignments, tabs pointing at objects nobody can read, and field security that the same container never grants object access for.
- **Troubleshooting "why can this person see that?"** Jump from any cell straight into the matching Salesforce Setup page.
