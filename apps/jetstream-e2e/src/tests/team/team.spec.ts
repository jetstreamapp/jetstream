...
test.describe('Team Dashboard', () => {
  // TODO: figure out how to test team creation since that relies on stripe etc..
  // see if we can mock some things somehow
  ...
  test('Team dashboard access', async ({ teamDashboardPage, teamCreationUtils3Users: teamCreationUtils, page }) => {
    ...
    await test.step('Verify team members', async () => {
      const row = page.getByTestId(`team-member-row-${adminUser.teamMembership.userId}`);
      ...
      const row1 = page.getByTestId(`team-member-row-${member1.teamMembership.userId}`);
      ...
      const row2 = page.getByTestId(`team-member-row-${member2.teamMembership.userId}`);
      ...
      const row3 = page.getByTestId(`team-member-row-${billingMember1.teamMembership.userId}`);
    ...
    await test.step('Verify billing user has limited access', async () => {
      ...
      const row1 = page.getByTestId(`team-member-row-${billingMember1.teamMembership.userId}`);
      ...
      const row2 = page.getByTestId(`team-member-row-${member1.teamMembership.userId}`);
      await expect(row2.getByText(member1.teamMembership.user.email, { exact: true })).toBeVisible();
      await expect(row2.getByTestId('user-row-actions')).toBeVisible();

      const row3 = page.getByTestId(`team-member-row-${member2.teamMembership.userId}`);
  ...
  test('Team invitations', async ({ authenticationPage, teamDashboardPage, teamCreationUtils1User: teamCreationUtils, page, browser }) => {
    ...
    const { user1Email, user2Email, teamInvites, users } = await test.step('Invite new team member', async () => {
      ...
      const users = [user1Email, user2Email];
      ...
      let row = page.getByTestId('team-invite-table').getByTestId(`team-member-row-invite-${user1Email}`);
      await expect(row.getByText(user1Email)).toBeVisible();
      await expect(row.getByText('Pending')).toBeVisible();

      await teamDashboardPage.inviteTeamMember(user2Email);
      row = page.getByTestId('team-invite-table').getByTestId(`team-member-row-invite-${user2Email}`);
    ...
    // TODO: register in advance, then log out, then accept invite during sign up

    // TODO: register in advance, then accept invite while logged in and ensure all restrictions are enforced correctly
  });
});
