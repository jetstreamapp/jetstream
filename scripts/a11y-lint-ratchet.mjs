...
// Run oxlint's own entry through node rather than the `pnpm` shim: spawning `pnpm` without a shell
// fails on Windows (the .cmd shim needs one), which would break pre-commit for Windows contributors.
const OXLINT_BIN = join(dirname(createRequire(import.meta.url).resolve('oxlint/package.json')), 'bin', 'oxlint');
...
function runOxlint() {
  ...
  let stdout;
  try {
    stdout = execFileSync(process.execPath, [OXLINT_BIN, '--format', 'json', '.'], options);
  } catch (error) {
  ...
  // Nx's boundary rule prints a plain-text warning ahead of the report when the project graph has not
  // been computed yet (a fresh clone or worktree). The rule is irrelevant here, so skip to the JSON.
  const reportStart = stdout.indexOf('{');
  if (reportStart === -1) {
    throw new Error(`oxlint did not print a JSON report:\n${stdout}`);
  }
  return JSON.parse(stdout.slice(reportStart)).diagnostics;
}
...
function readSourceWithLfEndings(path) {
...
function listSweptRouteKeys() {
  // Normalized so the line-anchored patterns below also match a CRLF checkout (Windows autocrlf)
  const routerSource = readSourceWithLfEndings(ROUTER_PATH);
  ...
  const sweepSource = readSourceWithLfEndings(PAGE_SWEEP_SPEC_PATH);
