...
function buildFileEditors(version, hash) {
  ...
  editors['Dockerfile'] = dockerArg;
  editors['Dockerfile.e2e'] = dockerArg;
...
async function main() {
  ...
  if (skipInstall) {
    console.log('\nSkipped lockfile refresh (--no-install). Run `corepack pnpm install` to update pnpm-lock.yaml.');
    return;
  }

  // Go through corepack so the lockfile is refreshed with the version we just pinned in package.json's
  // `packageManager`, not whatever pnpm happens to be on PATH (which could write a stale lockfile format).
  console.log('\nRefreshing pnpm-lock.yaml via `corepack pnpm install --lockfile-only`...');
  try {
    execFileSync('corepack', ['pnpm', 'install', '--lockfile-only'], {
      cwd: repoRoot,
      stdio: 'inherit',
      env: { ...process.env, COREPACK_ENABLE_DOWNLOAD_PROMPT: '0' },
    });
  } catch {
    console.warn('\n⚠ Could not run `corepack pnpm install` automatically. Run it manually to refresh pnpm-lock.yaml.');
  }
}
